-- set search_path to sm_sc;
-- drop function if exists public.trunc(sm_sc.typ_l_complex, int);
create or replace function public.trunc
(
  i_left     sm_sc.typ_l_complex    ,
  i_right    int
)
returns sm_sc.typ_l_complex
as
$$
-- declare 
begin
  return 
    (trunc(i_left.m_re :: decimal, i_right), trunc(i_left.m_im :: decimal, i_right))::sm_sc.typ_l_complex;
end
$$
language plpgsql stable
parallel safe
cost 100;
-- -- set search_path to sm_sc;
-- select public.trunc
--   (
--     (12.35464645, -12.30325015),
--     2
--   );

-- -------------------------------------------------------------------------------------
-- set search_path to sm_sc;
-- drop function if exists public.trunc(sm_sc.typ_l_complex);
create or replace function public.trunc
(
  i_right    sm_sc.typ_l_complex
)
returns sm_sc.typ_l_complex
as
$$
-- declare 
begin
  return (trunc(i_right.m_re :: decimal), trunc(i_right.m_im :: decimal))::sm_sc.typ_l_complex;
end
$$
language plpgsql stable
parallel safe
cost 100;
-- -- set search_path to sm_sc;
-- select public.trunc
--   (
--     (-45.6, -45.6)
--   );