...
begin
  ...
  ;
  v_ret := array_fill(null::int, v_len_depdt);
  ...
  then 
    return array_fill(null::int, array[coalesce(array_length(i_right, 1), 0), coalesce(array_length(i_right, 2), 0)]);
  -- [] compare array[]
  elsif v_len_right is null and i_right is not null -- = 0
  then 
    return array_fill(null::int, array[coalesce(array_length(i_left, 1), 0), coalesce(array_length(i_left, 2), 0)]);
      ...
      loop
        ...
        loop
          v_ret[v_y_cur][v_x_cur][v_x3_cur] := 
            sign(i_left[v_y_cur][v_x_cur][v_x3_cur] - i_right[v_y_cur][v_x_cur][v_x3_cur])
          ;
        end loop;    
      end loop;
    end loop;
        ...
        loop
          ...
          loop
            v_ret[v_y_cur][v_x_cur][v_x3_cur][v_x4_cur] := 
              sign(i_left[v_y_cur][v_x_cur][v_x3_cur][v_x4_cur] - i_right[v_y_cur][v_x_cur][v_x3_cur][v_x4_cur])
            ;
          end loop;
        end loop;    
      end loop;
...
begin
  ...
  then
    v_ret := array_fill(null::int, array[array_length(i_left, 1), array_length(i_left, 2)]);
  ...
  then
    v_ret := array_fill(null::int, array[array_length(i_left, 1)]);
    while v_y_cur <= array_length(i_left, 1)
    loop
  ...
  then
    v_ret := array_fill(null::int, array[array_length(i_left, 1), array_length(i_left, 2), array_length(i_left, 3)]);
    ...
    loop
      ...
      loop
        ...
        loop
          v_ret[v_y_cur][v_x_cur][v_x3_cur] := sign(i_left[v_y_cur][v_x_cur][v_x3_cur] - i_right);
        end loop;    
      end loop;
    end loop;
    return v_ret;
  ...
  then
    v_ret := array_fill(null::int, array[array_length(i_left, 1), array_length(i_left, 2), array_length(i_left, 3), array_length(i_left, 4)]);
    for v_y_cur in 1 .. array_length(i_left, 1)
    loop
      for v_x_cur in 1 .. array_length(i_left, 2)
      loop
        ...
        loop
          ...
          loop
            v_ret[v_y_cur][v_x_cur][v_x3_cur][v_x4_cur] := sign(i_left[v_y_cur][v_x_cur][v_x3_cur][v_x4_cur] - i_right);
          end loop;
        end loop;    
      end loop;
    end loop;
...
begin
  ...
  then
    v_ret := array_fill(null::int, array[array_length(i_right, 1), array_length(i_right, 2)]);
  ...
  then
    v_ret := array_fill(null::int, array[array_length(i_right, 1)]);
    while v_y_cur <= array_length(i_right, 1)
    loop
  ...
  then
    v_ret := array_fill(null::int, array[array_length(i_right, 1), array_length(i_right, 2), array_length(i_right, 3)]);
    ...
    loop
      ...
      loop
        ...
        loop
          v_ret[v_y_cur][v_x_cur][v_x3_cur] := sign(i_left - i_right[v_y_cur][v_x_cur][v_x3_cur]);
        end loop;    
      end loop;
    end loop;
    return v_ret;
  ...
  then
    v_ret := array_fill(null::int, array[array_length(i_right, 1), array_length(i_right, 2), array_length(i_right, 3), array_length(i_right, 4)]);
    for v_y_cur in 1 .. array_length(i_right, 1)
    loop
      for v_x_cur in 1 .. array_length(i_right, 2)
      loop
        ...
        loop
          ...
          loop
            v_ret[v_y_cur][v_x_cur][v_x3_cur][v_x4_cur] := sign(i_left - i_right[v_y_cur][v_x_cur][v_x3_cur][v_x4_cur]);
          end loop;
        end loop;    
      end loop;
    end loop;
