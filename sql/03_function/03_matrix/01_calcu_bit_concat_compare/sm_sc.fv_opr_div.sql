...
--   raise exception 'i_left: %; i_right: %; sqlerrm: %;', i_left, i_right, sqlerrm;
end
$$
...
-- drop function if exists sm_sc.fv_div_for_zero(int, int);
create or replace function sm_sc.fv_div_for_zero
(
  i_left       int   ,
  i_right      int
...
-- drop function if exists sm_sc.fv_div_for_zero(bigint, bigint);
create or replace function sm_sc.fv_div_for_zero
(
  i_left       bigint   ,
  i_right      bigint
...
-- drop function if exists sm_sc.fv_div_for_zero(decimal, decimal);
create or replace function sm_sc.fv_div_for_zero
(
  i_left       decimal   ,
  i_right      decimal
...
begin
  ...
  then 
    return sm_sc.fv_opr_div_py(i_left, i_right);
  else    
    v_len_depdt :=
      (
        select 
    ...
    then 
      ...
      loop
        i_left[v_y_cur] := sm_sc.fv_div_for_zero(i_left[v_y_cur], i_right[v_y_cur]);
        v_y_cur := v_y_cur + 1;
      end loop;
      return i_left;
    ...
    then
      ...
      loop 
        ...
        loop
          i_left[v_y_cur][v_x_cur] := sm_sc.fv_div_for_zero(i_left[v_y_cur][v_x_cur], i_right[v_y_cur][v_x_cur]);
          v_x_cur := v_x_cur + 1;
        end loop;
        v_y_cur := v_y_cur + 1;
      end loop;
        ...
        loop
          ...
          loop
            i_left[v_y_cur][v_x_cur][v_x3_cur] = 
              sm_sc.fv_div_for_zero(i_left[v_y_cur][v_x_cur][v_x3_cur], i_right[v_y_cur][v_x_cur][v_x3_cur]) 
            ;
          end loop;    
        end loop;
      end loop;
          ...
          loop
            ...
            loop
              i_left[v_y_cur][v_x_cur][v_x3_cur][v_x4_cur] = 
                sm_sc.fv_div_for_zero(i_left[v_y_cur][v_x_cur][v_x3_cur][v_x4_cur], i_right[v_y_cur][v_x_cur][v_x3_cur][v_x4_cur])
              ;
            end loop;
          end loop;    
        end loop;
...
begin
  if pg_typeof(i_right) = ('double precision[]' :: regtype)
  then 
    return sm_sc.fv_opr_div_py(i_right); 
