...
create or replace function sm_sc.fv_grad_width_2nd
(
  i_array        anyarray
)
returns anyarray
...
begin
  ...
  elsif array_length(i_array, 4) > 2
  then
    return
      i_array[ : ][ : ][ : ][3 : ] -` i_array[ : ][ : ][ : ][2 : array_length(i_array, 4) - 1] -` i_array[ : ][ : ][ : ][2 : array_length(i_array, 4) - 1] +` i_array[ : ][ : ][ : ][ : array_length(i_array, 4) - 2];
...
-- select sm_sc.fv_grad_width_2nd
--   (
