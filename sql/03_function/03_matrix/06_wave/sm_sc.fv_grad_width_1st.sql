...
create or replace function sm_sc.fv_grad_width_1st
(
  i_array        anyarray
)
returns anyarray
...
begin
  ...
  elsif array_length(i_array, 3) > 1
  then
    return
      i_array[ : ][ : ][2 : ] -` i_array[ : ][ : ][ : array_length(i_array, 3) - 1];
  
  elsif array_length(i_array, 4) > 1
  then
    return
      i_array[ : ][ : ][ : ][2 : ] -` i_array[ : ][ : ][ : ][ : array_length(i_array, 4) - 1];
...
-- select sm_sc.fv_grad_width_1st
--   (
