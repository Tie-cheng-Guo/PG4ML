...
-- --   v_len_1_ex     int   := case when array_ndims(i_array) = 1 then power(2, ceil(log(2, array_length(i_array, 1)))) :: int else null end;     -- 时域补零至就近的power(2,n)，频域升采样
-- --   v_w            sm_sc.typ_l_complex     :=   (exp(1) :: real ::sm_sc.typ_l_complex) ^ ((0.0 :: float, 2 * pi() / v_len_1_ex)::sm_sc.typ_l_complex);
...
-- --     end if;
-- --     
-- --     v_arr_even := sm_sc.fv_sgn_ifft(v_arr_even);    --  sm_sc.fv_sgn_ifft(v_arr_even[1] || (v_arr_even[2 : ] |+~| 1));
-- --     v_arr_odd := sm_sc.fv_sgn_ifft(v_arr_odd);     -- sm_sc.fv_sgn_ifft(v_arr_odd[1] || (v_arr_odd[2 : ] |+~| 1));
-- --     
...
begin 
  return 
    sm_sc.fv_mx_descend_dim_py(v_ifft[1 : 1]) :: sm_sc.typ_l_complex[] 
    +` 
    (sm_sc.fv_mx_descend_dim_py(v_ifft[2 : 2]) :: sm_sc.typ_l_complex[] *` ((0,1) :: sm_sc.typ_l_complex))
  ;
end
$$
