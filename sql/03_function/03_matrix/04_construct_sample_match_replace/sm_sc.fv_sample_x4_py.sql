...
create or replace function sm_sc.fv_sample_x4_py   -- 3 个参数
(
  i_array          float[]           
, i_period         int                                               -- 采样周期
...
declare
  ...
  v_cnt_window   int         := ((array_length(i_array, 4) - upper(i_simp_range) + 1) / i_period)  + 1;  -- array_length(i_array, x4)
begin 
  ...
  then 
    return 
      ...
      (
        (sm_sc.fv_concat_py    -- fv_concat_x5
        (
          sm_sc.fv_concat_py   -- fv_concat_x5
          (
            array_fill
            (
              i_fill_value
            , array[array_length(i_array, 1), array_length(i_array, 2), array_length(i_array, 3), v_cnt_window, least(i_window_len, lower(i_simp_range) - 1)]
            )
        ...
        , array_fill
          (
            i_fill_value
          , array[array_length(i_array, 1), array_length(i_array, 2), array_length(i_array, 3), v_cnt_window, greatest(0, i_window_len - upper(i_simp_range) + 1)]
          )
        , 5
        ))[ : ][ : ][ : ][ : ][ : i_window_len]
      , array
...
-- )
-- select 
--   sm_sc.fv_sample_x4_py(a_arr, 2, a_no, int4range(1, 3, '[]'), 0.0)
--   =
--   sm_sc.fv_concat_x4(sm_sc.fv_concat_x4(a_arr[ : ][ : ][ : ][1: 1 + a_no - 1], a_arr[ : ][ : ][ : ][3:3 + a_no - 1]), a_arr[ : ][ : ][ : ][5:5 + a_no - 1])
-- from cte_arr
--   , generate_series(1, 3) tb_a(a_no)

-- with cte_arr as 
-- (
--    select sm_sc.fv_new_rand(array[2,3,6,8]) as a_arr
-- )
-- select 
--   a_no
-- , array_dims(sm_sc.fv_sample_x4_py(a_arr, 2, a_no, int4range(1, 3, '[]'), 0.0))
-- from cte_arr
