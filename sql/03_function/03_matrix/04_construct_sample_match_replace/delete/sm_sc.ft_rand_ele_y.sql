...
create or replace function sm_sc.ft_rand_ele_y
(
  i_array          anyarray          ,
  i_select_cnt     int
)
  ...
  then
    return query
      ...
      (
        select 
          a_cur_y,
          sm_sc.fv_rand_1d_ele(array_length(i_array, 2), i_select_cnt) as a_cur_xs
      ...
      (
        ...
        from generate_series(1, i_select_cnt) tb_a_cur_new_x(a_cur_new_x), cte_cur_xs tb_a_cur_y
      ...
      select 
        ...
        -- -- array_agg(a_y_eles order by a_cur_y)
      from cte_new_y
    ;
  else
...
cost 100;
-- -- set search_path to sm_sc;
-- select * from sm_sc.ft_rand_ele_y
--   (
