...
create or replace function sm_sc.fv_d_bpad_dloss_dindepdt_2
(
...
$$
declare 
  v_len     int       := array_length(i_dloss_ddepdt, array_ndims(i_dloss_ddepdt) - 1);
begin
  -- set search_path to sm_sc;
  ...
  then
    ...
    (
      select 
        sm_sc.fa_mx_sum(i_dloss_ddepdt[v_len - (a_cur * i_indepdt_heigh) + 1 : v_len - ((a_cur - 1) * i_indepdt_heigh)][ : ])
...
-- -- set search_path to sm_sc;
-- select sm_sc.fv_d_bpad_dloss_dindepdt_2
--   (
--     array
--     [
--       [1, 2, 3, 4, 5]
