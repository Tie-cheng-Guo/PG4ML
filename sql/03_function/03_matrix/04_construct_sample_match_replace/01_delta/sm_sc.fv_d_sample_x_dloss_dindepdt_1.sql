...
create or replace function sm_sc.fv_d_sample_x_dloss_dindepdt_1
(
  i_dloss_ddepdt   float[]      ,                -- 损失函数对因变量的导数
  i_array_len_x    int          ,                -- 自变量数组在采样维度的长度
  -- i_period         int          ,                -- 采样周期长度
...
begin
  ...
  then 
    ...
    elsif array_length(i_dloss_ddepdt, 2) % v_window_len_ex > 0
      or i_array_len_x / (array_length(i_dloss_ddepdt, 2) % v_window_len_ex) > 0
    then 
      raise exception 'unmatch 2d length of i_dloss_ddepdt, simp_range or i_array_len for period.';
    end if;
  end if;
  ...
  -- 换算出 i_array 周期
  v_period  :=   i_array_len_x / (array_length(i_dloss_ddepdt, 2) / v_window_len_ex);
  
  if array_ndims(i_dloss_ddepdt) = 2
  then
    v_ret     :=   array_fill(0.0, array_length(i_dloss_ddepdt, 1) || array[i_array_len_x]);
  elsif array_ndims(i_dloss_ddepdt) = 3
  then
    v_ret     :=   array_fill(0.0, array_length(i_dloss_ddepdt, 1) || array[i_array_len_x] || array_length(i_dloss_ddepdt, 3));
  elsif array_ndims(i_dloss_ddepdt) = 4
  then
    v_ret     :=   array_fill(0.0, array_length(i_dloss_ddepdt, 1) || array[i_array_len_x] || array_length(i_dloss_ddepdt, 3) || array_length(i_dloss_ddepdt, 4));
  end if;
  ...
  for v_cur_1 in 1 .. (array_length(i_dloss_ddepdt, 2) / v_window_len_ex)
  loop 
    for v_cur_2 in 1 .. array_length(v_arr_idxs, 1)
    loop 
      if (v_cur_1 - 1) * v_period + v_arr_idxs[v_cur_2] between 1 and array_length(v_ret, 2)
      then 
...
cost 100;
-- -- set search_path to sm_sc;
-- select sm_sc.fv_d_sample_x_dloss_dindepdt_1
--   (
--     array
--     [
--       [1, 2, 3, 4, 5, 6, 7, 8, 9]
