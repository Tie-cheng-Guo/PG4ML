...
create or replace function sm_sc.fv_d_slice_x4_dloss_dindepdt
(
...
begin
  ...
  then 
    ...
    elsif i_arr_len_x4 < (select max(upper(a_range)) - 1 from unnest(i_slice_range) tb_a_range(a_range))
      or 1 > (select min(lower(a_range)) from unnest(i_slice_range) tb_a_range(a_range))
    then
      raise exception 'overflow range for i_arr_len_x4.';
    elsif array_length(i_dloss_ddepdt, 4) <> (select sum(upper(a_range) - lower(a_range)) from unnest(i_slice_range) tb_a_range(a_range))
    then 
  ...
  then 
    v_ret := array_fill(0.0, array[array_length(i_dloss_ddepdt, 1), array_length(i_dloss_ddepdt, 2), array_length(i_dloss_ddepdt, 3)] || i_arr_len_x4);
  end if;
  ...
  for v_cur in 
    ...
    (
      select 
        a_arr_idx,
        a_range_idx
      from generate_series(1, i_arr_len_x4) tb_a_cur_idx(a_arr_idx)
...
-- select sm_sc.fv_d_slice_x4_dloss_dindepdt
--   (
