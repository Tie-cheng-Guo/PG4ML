...
create or replace function sm_sc.fv_d_apad_dloss_dindepdt_1
(
...
begin
  ...
  then 
    return 
      i_dloss_ddepdt[array_length(i_dloss_ddepdt, 1) - i_indepdt_heigh + 1 : ][ : ]
    ;
  elsif array_ndims(i_dloss_ddepdt) = 3
  then 
    return 
      i_dloss_ddepdt[ : ][array_length(i_dloss_ddepdt, 2) - i_indepdt_heigh + 1 : ][ : ]
    ;
  elsif array_ndims(i_dloss_ddepdt) = 4
  then 
    return 
      i_dloss_ddepdt[ : ][ : ][array_length(i_dloss_ddepdt, 3) - i_indepdt_heigh + 1 : ][ : ]
    ;
  end if;
end
$$
...
-- -- set search_path to sm_sc;
-- select sm_sc.fv_d_apad_dloss_dindepdt_1
--   (
