...
create or replace function sm_sc.fv_amask
(
  i_array               anyarray    ,
  i_mask_len            int[]       ,     -- 该控制参数的维度与 i_array 相同，背景面的 heigh 维度(次高维度)长度为 1，其他维度长度与 i_array 对应维度长度也相同。
    ...
    (
      select 
        ...
        (
          array 
          [
            nullif(int4range(1, i_mask_len[1][a_cur_x] + 1, '[)'), 'empty' :: int4range)
    ...
    (
      select 
        ...
        (
          ...
          , nullif(int4range(1, i_mask_len[a_cur_y][1][a_cur_x3] + 1, '[)'), 'empty' :: int4range)
      ...
      select 
        ...
        (
          ...
          [
            int4range(a_cur_y, a_cur_y, '[]')
          , int4range(a_cur_x, a_cur_x, '[]')
          , nullif(int4range(1, i_mask_len[a_cur_y][a_cur_x][1][a_cur_x4] + 1, '[)'), 'empty' :: int4range)
