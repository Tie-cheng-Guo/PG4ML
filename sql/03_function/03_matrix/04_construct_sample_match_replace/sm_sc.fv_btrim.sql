...
create or replace function sm_sc.fv_btrim
(
  i_array               anyarray,
...
begin
  ...
  then
    ...
    loop 
      for v_y_cur in reverse array_length(i_array, 1) .. 1
      loop 
  ...
  then
    for v_y_cur in reverse array_length(i_array, 1) .. 1
    loop 
      if i_array[v_y_cur] <> i_trim_element
      then 
        exit;
      else 
        i_array[v_y_cur] = null;
      end if;
    end loop;
    return i_array;
  ...
  then
    ...
    loop 
      ...
      loop 
        ...
        loop 
          for v_x3_cur in reverse array_length(i_array, 3) .. 1
          loop 
...
parallel safe
cost 100;
-- select sm_sc.fv_btrim(array[[2,0,0,0,0,0], [1,2,0,4,0,0], [0,0,1,0,2,0], [0,1,2,0,0,0], [1,0,1,0,0,3]], 0)
-- select sm_sc.fv_btrim(array[0,1,2,3,4,5, 0, 0], 0)