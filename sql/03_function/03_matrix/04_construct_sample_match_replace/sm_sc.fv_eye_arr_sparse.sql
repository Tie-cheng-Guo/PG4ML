...
create or replace function sm_sc.fv_eye_arr_sparse
(
...
begin
  ...
  then
    ...
    -- 计算结果矩阵的长宽
    -- v_ret_len = (select sum(greatest(array_length(i_arrays[col_cur].m_array, 1), array_length(i_arrays[col_cur].m_array, 2))) from generate_subscripts(i_arrays, 1) tb(col_cur));
    ...
    (
      select 
        col_cur
      , sum(greatest(len_y, len_x)) over(order by col_cur) as cur_squar 
      from cte_shape
    ),
    -- 每一层方阵右下角作为下一层起点
    ...
    -- 所有层坐标的最大值即返回结果矩阵的形状，且保证返回矩阵为方阵
    select 
      greatest(max(cur_end_pos_y), max(cur_end_pos_x))
    into v_ret_len_y  -- , v_ret_len_x
    from cte_cur_end_pos;
    
    v_ret_len_x := v_ret_len_y;
    ...
    loop
      ...
      v_cur_pos = v_cur_pos + greatest(array_length(v_cur_arr.m_array, 1), array_length(v_cur_arr.m_array, 2));
    end loop;
...
parallel safe
  cost 100;
-- select sm_sc.fv_eye_arr_sparse(0.5, array[[1, 1], [1, 1.0]] :: float[], array[[2, 2, 2, 2], [2, 2, 2, 2], [2, 2, 2, 2], [2, 2, 2, 2]] :: float[], array[[3, 3, 3], [3, 3, 3], [3, 3, 3]] :: float[])
-- select sm_sc.fv_eye_arr_sparse(0.8, array[[1.2, 2.4], [11.2, 12.4]] :: float[], array[[1.3, 2.6, 1.4, 2.8], [11.3, 12.6, 11.4, 12.8]] :: float[], array[[1.1, 2.2], [11.1, 12.2], [21.1, 22.2]] :: float[])
-- ---------------------------
-- drop function if exists sm_sc.fv_eye_arr_sparse(variadic sm_sc.__typ_array_ex[]);
create or replace function sm_sc.fv_eye_arr_sparse
(
...
-- declare -- here
begin
  return sm_sc.fv_eye_arr_sparse(0.0 :: float, variadic i_arrays);
end
$$
  language plpgsql stable
parallel safe
  cost 100;
-- select sm_sc.fv_eye_arr_sparse(array[[1.0, 1], [1, 1]] :: float[], array[[2.0, 2, 2, 2], [2, 2, 2, 2], [2, 2, 2, 2], [2, 2, 2, 2]] :: float[], array[[3.0, 3, 3], [3, 3, 3], [3, 3, 3]] :: float[])
-- select sm_sc.fv_eye_arr_sparse(array[[1.2, 2.4], [11.2, 12.4]] :: float[], array[[1.3, 2.6, 1.4, 2.8], [11.3, 12.6, 11.4, 12.8]] :: float[], array[[1.1, 2.2], [11.1, 12.2], [21.1, 22.2]] :: float[])