...
create or replace function sm_sc.ft_rand_slice_x_pick
(
  i_array          anyarray          ,
  i_pick_cnt       int
)
...
begin
  ...
  then 
    return query
      select null, i_array;
  elsif i_pick_cnt <= array_length(i_array, 2)
  then 
    if array_ndims(i_array) = 2 and i_pick_cnt <= array_length(i_array, 2)
    then
      return query
        select
          array_agg(a_cur) as o_ord_nos,
          sm_sc.fa_mx_concat_x(i_array[ : ][a_cur : a_cur]) as o_slices
        from unnest(sm_sc.fv_rand_1d_ele_pick(array_length(i_array, 2), i_pick_cnt)) tb_a_cur(a_cur)
      ;
    elsif array_ndims(i_array) = 3 and i_pick_cnt <= array_length(i_array, 2)
    then
      return query
        select
          array_agg(a_cur) as o_ord_nos,
          sm_sc.fa_mx_concat_x(i_array[ : ][a_cur : a_cur][ : ]) as o_slices
        from unnest(sm_sc.fv_rand_1d_ele(array_length(i_array, 2), i_pick_cnt)) tb_a_cur(a_cur)
      ;
    elsif array_ndims(i_array) = 4 and i_pick_cnt <= array_length(i_array, 2)
    then
      return query
        select
          array_agg(a_cur) as o_ord_nos,
...
cost 100;
-- -- set search_path to sm_sc;
-- select * from sm_sc.ft_rand_slice_x_pick
--   (
