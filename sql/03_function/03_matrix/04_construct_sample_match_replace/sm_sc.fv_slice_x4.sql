...
create or replace function sm_sc.fv_slice_x4
(
  i_arr            anyarray,
...
begin
  ...
  then 
    if array_length(i_arr, 4) < (select max(upper(a_range)) - 1 from unnest(i_slice_range) tb_a_range(a_range))
      or 1 > (select min(lower(a_range)) from unnest(i_slice_range) tb_a_range(a_range))
    then
      raise exception 'overflow range for 4d len of i_arr.';
    end if;
  end if;
