...
create or replace function sm_sc.__fv_turn_y_x4_90
(
  i_right     anyarray
)
returns anyarray
...
begin
  ...
  then
    v_ret := array_fill(nullif(i_right[1][1][1][1], i_right[1][1]), array[v_len_x4, v_len_x, v_len_x3, v_len_y]);
    for v_cur_1 in 1 .. v_len_y
    loop 
...
cost 100;
-- -- set search_path to sm_sc;
-- -- select sm_sc.__fv_turn_y_x4_90
-- --   (
