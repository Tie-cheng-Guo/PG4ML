...
create or replace function sm_sc.fv_opr_turn_width_heigh_90
(
  i_right     anyarray
)
returns anyarray
...
begin
  ...
  then
    ...
    -- (
    --   select
    --     array_agg(array_x_new order by a_cur_x desc)
    --   from 
    --   (
    --     select 
    --       a_cur_x,
    --       array_agg(i_right[a_cur_y][a_cur_x] order by a_cur_y) as array_x_new
    ...
    for v_cur_1 in 1 .. v_len_y
    loop 
      v_ret[ : ][v_len_y - v_cur_1 + 1 : v_len_y - v_cur_1 + 1] := i_right[v_cur_1 : v_cur_1][ : ];
    end loop;
    return v_ret;
    
  elsif array_ndims(i_right) = 3
  then
    v_ret := array_fill(nullif(i_right[1][1][1], i_right[1][1][1]), array[v_len_y, v_len_x3, v_len_x]);
    for v_cur_1 in 1 .. v_len_x
    loop 
      for v_cur_2 in 1 .. v_len_x3
      loop 
        v_ret[ : ][v_cur_2 : v_cur_2][v_len_x - v_cur_1 + 1 : v_len_x - v_cur_1 + 1] := i_right[ : ][v_cur_1 : v_cur_1][v_cur_2 : v_cur_2];
      end loop;
    end loop;
    return v_ret;
  ...
  then
    v_ret := array_fill(nullif(i_right[1][1][1][1], i_right[1][1]), array[v_len_y, v_len_x, v_len_x4, v_len_x3]);
    for v_cur_1 in 1 .. v_len_x3
    loop 
      for v_cur_2 in 1 .. v_len_x4
      loop 
        v_ret[ : ][ : ][v_cur_2 : v_cur_2][v_len_x3 - v_cur_1 + 1 : v_len_x3 - v_cur_1 + 1] := i_right[ : ][ : ][v_cur_1 : v_cur_1][v_cur_2 : v_cur_2];
      end loop;
    end loop;
    return v_ret;
...
cost 100;
-- -- set search_path to sm_sc;
-- select sm_sc.fv_opr_turn_width_heigh_90
--   (
