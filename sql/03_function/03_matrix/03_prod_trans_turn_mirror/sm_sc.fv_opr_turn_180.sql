...
create or replace function sm_sc.fv_opr_turn_180
(
  i_arr            anyarray,
  i_dims           int[2]     default null
)
returns anyarray
as
$$
...
begin
  ...
  then 
    return sm_sc.fv_opr_turn_180_py(i_arr, i_dims);
  else 
    if i_dims in (array[1, 2], array[2, 1])
    then 
      return sm_sc.__fv_turn_y_x_180(i_arr);    
    elsif i_dims in (array[1, 3], array[3, 1])
    then
      return sm_sc.__fv_turn_y_x3_180(i_arr);   
    elsif i_dims in (array[1, 4], array[4, 1])
    then
      return sm_sc.__fv_turn_y_x4_180(i_arr);   
    elsif i_dims in (array[2, 3], array[3, 2])
    then
      return sm_sc.__fv_turn_x_x3_180(i_arr);   
    elsif i_dims in (array[2, 4], array[4, 2])
    then
      return sm_sc.__fv_turn_x_x4_180(i_arr);   
    elsif i_dims in (array[3, 4], array[4, 3])
    then
      return sm_sc.__fv_turn_x3_x4_180(i_arr);
    end if;
  end if;
end
$$
