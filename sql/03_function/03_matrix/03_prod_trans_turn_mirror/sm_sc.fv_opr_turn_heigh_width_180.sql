...
create or replace function sm_sc.fv_opr_turn_heigh_width_180
(
  i_right     anyarray
)
returns anyarray
...
begin
  ...
  then
    ...
    -- )
    -- ;
    v_ret := array_fill(nullif(i_right[1][1], i_right[1][1]), array[v_len_y, v_len_x]);
    for v_cur_1 in 1 .. v_len_y
    loop 
      for v_cur_2 in 1 .. v_len_x
      loop 
        v_ret[v_len_y - v_cur_1 + 1 : v_len_y - v_cur_1 + 1]
             [v_len_x - v_cur_2 + 1 : v_len_x - v_cur_2 + 1] 
          := i_right[v_cur_1 : v_cur_1]
                    [v_cur_2 : v_cur_2]
        ;
      end loop;
    end loop;
  ...
  then
    v_ret := array_fill(nullif(i_right[1][1][1], i_right[1][1][1]), array[v_len_y, v_len_x, v_len_x3]);
    for v_cur_1 in 1 .. v_len_x
    loop 
      for v_cur_2 in 1 .. v_len_x3
      loop 
        v_ret[ : ]
             [v_len_x - v_cur_1 + 1 : v_len_x - v_cur_1 + 1]
             [v_len_x3 - v_cur_2 + 1 : v_len_x3 - v_cur_2 + 1]
          := i_right[ : ]
                    [v_cur_1 : v_cur_1]
                    [v_cur_2 : v_cur_2]
        ;
  ...
  then
    v_ret := array_fill(nullif(i_right[1][1][1][1], i_right[1][1]), array[v_len_y, v_len_x, v_len_x3, v_len_x4]);
    ...
    loop 
      ...
      loop 
        v_ret[ : ]
             [ : ]
             [v_len_x3 - v_cur_1 + 1 : v_len_x3 - v_cur_1 + 1]
             [v_len_x4 - v_cur_2 + 1 : v_len_x4 - v_cur_2 + 1]
          := i_right[ : ]
                    [ : ]
                    [v_cur_1 : v_cur_1]
                    [v_cur_2 : v_cur_2]
...
cost 100;
-- -- set search_path to sm_sc;
-- select sm_sc.fv_opr_turn_heigh_width_180
--   (
