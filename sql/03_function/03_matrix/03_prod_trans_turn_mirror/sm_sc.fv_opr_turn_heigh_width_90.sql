...
create or replace function sm_sc.fv_opr_turn_heigh_width_90
(
  i_right     anyarray
)
returns anyarray
...
begin
  ...
  then
    ...
    for v_cur_1 in 1 .. v_len_x4
    loop 
      for v_cur_2 in 1 .. v_len_x3
      loop 
        v_ret[ : ][ : ][v_len_x4 - v_cur_1 + 1 : v_len_x4 - v_cur_1 + 1][v_cur_2 : v_cur_2] := i_right[ : ][ : ][v_cur_2 : v_cur_2][v_cur_1 : v_cur_1];
      end loop;
    end loop;
    return v_ret;
  else
...
cost 100;
-- -- set search_path to sm_sc;
-- select sm_sc.fv_opr_turn_heigh_width_90
--   (
