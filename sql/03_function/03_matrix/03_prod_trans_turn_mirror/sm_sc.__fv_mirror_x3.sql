...
create or replace function sm_sc.__fv_mirror_x3
(
  i_right     anyarray
)
returns anyarray
...
declare -- here
  ...
  -- v_ele              alias for $0;
  v_ret               i_right%type;
  v_len_mirror_dim    int  :=   array_length(i_right, 3);
begin
  -- set search_path to sm_sc;
  ...
  -- --   -- -- )
  -- --   -- -- ;
  -- --   v_ret := array_fill(nullif(i_right[1][1], i_right[1][1]), array[array_length(i_right, 1), array_length(i_right, 2)]);
