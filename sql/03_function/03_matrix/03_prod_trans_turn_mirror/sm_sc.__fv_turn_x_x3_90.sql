...
create or replace function sm_sc.__fv_turn_x_x3_90
(
  i_right     anyarray
)
returns anyarray
...
begin
  ...
  then
    v_ret := array_fill(nullif(i_right[1][1][1][1], i_right[1][1]), array[v_len_y, v_len_x3, v_len_x, v_len_x4]);
    for v_cur_1 in 1 .. v_len_x
    loop 
      for v_cur_2 in 1 .. v_len_x3
      loop 
...
cost 100;
-- -- set search_path to sm_sc;
-- -- select sm_sc.__fv_turn_x_x3_90
-- --   (
