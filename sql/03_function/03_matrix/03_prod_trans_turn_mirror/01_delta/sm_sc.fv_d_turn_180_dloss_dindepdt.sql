-- set search_path to sm_sc;
-- drop function if exists sm_sc.fv_d_turn_180_dloss_dindepdt(anyarray, int[2]);
create or replace function sm_sc.fv_d_turn_180_dloss_dindepdt
(
  i_dloss_ddepdt            anyarray,
  i_dims           int[2]
)
returns anyarray
as
$$
-- declare 
begin
  -- set search_path to sm_sc;
  if i_dims in (array[1, 2], array[2, 1])
  then 
    return sm_sc.__fv_turn_y_x_180(i_dloss_ddepdt);    
  elsif i_dims in (array[1, 3], array[3, 1])
  then
    return sm_sc.__fv_turn_y_x3_180(i_dloss_ddepdt);   
  elsif i_dims in (array[1, 4], array[4, 1])
  then
    return sm_sc.__fv_turn_y_x4_180(i_dloss_ddepdt);   
  elsif i_dims in (array[2, 3], array[3, 2])
  then
    return sm_sc.__fv_turn_x_x3_180(i_dloss_ddepdt);   
  elsif i_dims in (array[2, 4], array[4, 2])
  then
    return sm_sc.__fv_turn_x_x4_180(i_dloss_ddepdt);   
  elsif i_dims in (array[3, 4], array[4, 3])
  then
    return sm_sc.__fv_turn_x3_x4_180(i_dloss_ddepdt);
  end if;
end
$$
language plpgsql stable
parallel safe
cost 100;
-- -- set search_path to sm_sc;

-- select sm_sc.fv_d_turn_180_dloss_dindepdt
--   (
--     array[[[[1,2,3,4],[11,12,13,14]],[[5,6,7,8],[15,16,17,18]]],[[[21,22,23,24],[31,32,33,34]],[[25,26,27,28],[35,36,37,38]]]]
--   , array[3, 4]
--   );