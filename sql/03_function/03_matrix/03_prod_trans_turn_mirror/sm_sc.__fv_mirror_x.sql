...
create or replace function sm_sc.__fv_mirror_x
(
  i_right     anyarray
)
returns anyarray
...
declare -- here
  ...
  -- v_ele              alias for $0;
  v_ret               i_right%type;
  v_len_mirror_dim    int  :=   array_length(i_right, 2);
begin
  -- set search_path to sm_sc;
  ...
  then
    v_ret := array_fill(nullif(i_right[1][1][1][1], i_right[1][1][1][1]), array[array_length(i_right, 1), v_len_mirror_dim, array_length(i_right, 3), array_length(i_right, 4)]);
    for v_cur in 1 .. v_len_mirror_dim
    loop 
      v_ret[ : ][v_cur : v_cur][ : ][ : ] := i_right[ : ][v_len_mirror_dim - v_cur + 1 : v_len_mirror_dim - v_cur + 1][ : ][ : ];
    end loop;
