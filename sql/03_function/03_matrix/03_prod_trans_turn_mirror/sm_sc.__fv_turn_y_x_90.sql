...
create or replace function sm_sc.__fv_turn_y_x_90
(
  i_right     anyarray
)
returns anyarray
...
begin
  ...
  then
    v_ret := array_fill(nullif(i_right[1][1][1], i_right[1][1][1]), array[v_len_x, v_len_y, v_len_x3]);
    for v_cur_1 in 1 .. v_len_y
    loop 
      for v_cur_2 in 1 .. v_len_x
      loop 
        v_ret[v_cur_2 : v_cur_2][v_len_y - v_cur_1 + 1 : v_len_y - v_cur_1 + 1][ : ] := i_right[v_cur_1 : v_cur_1][v_cur_2 : v_cur_2][ : ];
      end loop;
    end loop;
    return v_ret;
  ...
  then
    v_ret := array_fill(nullif(i_right[1][1][1][1], i_right[1][1]), array[v_len_x, v_len_y, v_len_x3, v_len_x4]);
    for v_cur_1 in 1 .. v_len_y
    loop 
      for v_cur_2 in 1 .. v_len_x
      loop 
