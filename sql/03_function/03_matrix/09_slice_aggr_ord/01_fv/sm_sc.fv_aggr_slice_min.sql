...
create or replace function sm_sc.fv_aggr_slice_min
(
  i_array          anyarray
)
returns anyelement
...
cost 100;
-- -- set search_path to sm_sc;
-- select sm_sc.fv_aggr_slice_min
--   (
...
begin
  ...
  then
    ...
    -- --       array_agg
    -- --       (
    -- --         sm_sc.fv_aggr_slice_min(i_array[a_cur_y : a_cur_y + i_cnt_per_grp[1] - 1][a_cur_x : a_cur_x + i_cnt_per_grp[2] - 1]) 
    -- --         order by a_cur_x
    -- --       ) as a_slice_x
