...
-- drop function if exists sm_sc.fv_aggr_slice_max_py(float[]);
create or replace function sm_sc.fv_aggr_slice_max_py
(
  i_array          float[]
)
returns float
...
$$
  import numpy as np
  return np.nanmax(np.float32(i_array)).tolist()
$$
...
-- ------------------------------------------------------------------
-- drop function if exists sm_sc.fv_aggr_slice_max_py(int[]);
create or replace function sm_sc.fv_aggr_slice_max_py
(
  i_array          int[]
)
...
$$
  import numpy as np
  return np.nanmax(np.int32(i_array)).tolist()
$$
language plpython3u stable
parallel safe
cost 100;
...
-- ------------------------------------------------------------------
-- drop function if exists sm_sc.fv_aggr_slice_max_py(bigint[]);
create or replace function sm_sc.fv_aggr_slice_max_py
(
  i_array          bigint[]
)
...
$$
  import numpy as np
  return np.nanmax(np.int64(i_array)).tolist()
$$
language plpython3u stable
parallel safe
cost 100;
...
-- -- set search_path to sm_sc;
-- select sm_sc.fv_aggr_slice_max_py
--   (
...
-- ------------------------------------------------------------------------------------------------------
-- drop function if exists sm_sc.fv_aggr_slice_max_py(float[], int[]);
create or replace function sm_sc.fv_aggr_slice_max_py
(
  i_array          float[],
  i_cnt_per_grp    int[]
...
$$
  ...
  if v_array.ndim == 1 :
    ...
    return \
      np.nanmax(np.nanmax(np.nanmax(np.nanmax( \
      np.lib.stride_tricks.as_strided(v_array \
          , shape = ((np.concatenate((v_array.shape // v_cnt_per_grp, v_cnt_per_grp), axis = 0)).tolist()) \
          , strides = ( \
...
-- )
-- select 
--   sm_sc.fv_aggr_slice_max_py(a_arr, array[3,7])
-- = sm_sc.fv_aggr_slice_max(a_arr, array[3,7])
-- from cte_arr
...
-- )
-- select 
--   sm_sc.fv_aggr_slice_max_py(a_arr, array[3,7,4])
-- = sm_sc.fv_aggr_slice_max(a_arr, array[3,7,4])
-- from cte_arr
...
-- )
-- select 
--   sm_sc.fv_aggr_slice_max_py(a_arr, array[3,7,4,5])
-- = sm_sc.fv_aggr_slice_max(a_arr, array[3,7,4,5])
-- from cte_arr



-- ------------------------------------------------------------------------------------------------------
-- drop function if exists sm_sc.fv_aggr_slice_max_py(int[], int[]);
create or replace function sm_sc.fv_aggr_slice_max_py
(
  i_array          int[],
  i_cnt_per_grp    int[]
...
-- ------------------------------------------------------------------------------------------------------
-- drop function if exists sm_sc.fv_aggr_slice_max_py(bigint[], int[]);
create or replace function sm_sc.fv_aggr_slice_max_py
(
  i_array          bigint[],
  i_cnt_per_grp    int[]
