...
create or replace function sm_sc.fv_aggr_slice_sum
(
  i_array          anyarray
)
returns anyelement
...
cost 100;
-- -- set search_path to sm_sc;
-- select sm_sc.fv_aggr_slice_sum
--   (
...
begin
  i_cnt_per_grp :=
    sm_sc.fv_coalesce
    (
      v_array_len[ : array_length(v_array_len, 1) - coalesce(array_length(i_cnt_per_grp, 1), 0)] || i_cnt_per_grp
    , v_array_len
    )
  ;
  ...
  then
    ...
    -- --       array_agg
    -- --       (
    -- --         sm_sc.fv_aggr_slice_sum(i_array[a_cur_y : a_cur_y + i_cnt_per_grp[1] - 1][a_cur_x : a_cur_x + i_cnt_per_grp[2] - 1]) 
    -- --         order by a_cur_x
    -- --       ) as a_slice_x
