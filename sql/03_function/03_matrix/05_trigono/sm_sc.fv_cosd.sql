...
create or replace function sm_sc.fv_cosd
(
...
begin
  ...
  then
    ...
    loop 
      ...
      loop
        i_array[v_y_cur][v_x_cur] := cosd(i_array[v_y_cur][v_x_cur]);
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
  ...
  then
    while v_y_cur <= array_length(i_array, 1)
    loop
      i_array[v_y_cur] := cosd(i_array[v_y_cur]);
      v_y_cur := v_y_cur + 1;
    end loop;
    return i_array;
      ...
      loop
        ...
        loop
          i_array[v_y_cur][v_x_cur][v_x3_cur] = 
            cosd(i_array[v_y_cur][v_x_cur][v_x3_cur])
          ;
        end loop;    
      end loop;
    end loop;
        ...
        loop
          ...
          loop
            i_array[v_y_cur][v_x_cur][v_x3_cur][v_x4_cur] = 
              cosd(i_array[v_y_cur][v_x_cur][v_x3_cur][v_x4_cur])
            ;
          end loop;
        end loop;    
      end loop;
