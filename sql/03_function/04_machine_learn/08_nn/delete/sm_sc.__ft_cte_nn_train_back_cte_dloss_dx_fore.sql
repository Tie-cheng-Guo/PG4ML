...
create or replace function __ft_cte_nn_train_back_cte_dloss_dindepdt_fore
(
  i_work_no             bigint              ,
  i_cur_nn_depth        int
)
returns -- sm_sc.fv_computational_graph_serialize   return jsonb
  ...
  (
    ...
    a_dloss_dindepdt_fore_1st            float[]    ,
    a_dloss_dindepdt_fore_2nd            float[]    ,
    ...
    select 
      ...
      case 
        when sm_sc.tb_fore.node_fn_type in ('agg_max', 'agg_min', 'agg_prod', 'softmax_x', 'zscore_x', 'agg_concat_x', 'agg_concat_y')
          then (sm_sc.tb_fore.node_o).m_dloss_ddepdt
        when sm_sc.tb_fore.node_fn_type = 'prod_mx' and sm_sc.tb_back_p1.is_back_node
          then (sm_sc.tb_fore.node_o).m_dloss_ddepdt |**| (|^~| (sm_sc.tb_fore.node_o).m_dy_d1st)
        when sm_sc.tb_fore.node_fn_type = 'conv_2d' and sm_sc.tb_back_p1.is_back_node
          then 
            ...
            (
              ...
              (sm_sc.tb_fore.node_o).m_dloss_ddepdt
            )            
        ...
        when sm_sc.tb_fore.node_fn_type = 'agg_avg' and sm_sc.tb_back_p1.is_back_node
          then 
            ...
            (
              ...
              1                             ,
              null                          ,
              array[sm_sc.tb_fore.node_depdt_len :: float[]] -- -- sm_sc.tb_fore.node_fn_asso_value 
            )
        ...
        else case when sm_sc.tb_back_p1.is_back_node then (sm_sc.tb_fore.node_o).m_dloss_ddepdt *` (sm_sc.tb_fore.node_o).m_dy_d1st else null end
      end
        as a_dloss_dindepdt_fore_1st,
      case
        when sm_sc.tb_fore.node_fn_type = 'prod_mx' and sm_sc.tb_back_p2.is_back_node
          then (sm_sc.tb_fore.node_o).m_dy_d2nd |**| (sm_sc.tb_fore.node_o).m_dloss_ddepdt
        ...
        when sm_sc.tb_fore.node_fn_type in ('agg_concat_y', 'agg_concat_x', 'rand_pick_y', 'rand_pick_x', 'new')
          ...
          then null
        else case when sm_sc.tb_back_p2.is_back_node then (sm_sc.tb_fore.node_o).m_dloss_ddepdt *` (sm_sc.tb_fore.node_o).m_dy_d2nd else null end
      end
        as a_dloss_dindepdt_fore_2nd,
      sm_sc.tb_fore.node_fn_asso_value,
