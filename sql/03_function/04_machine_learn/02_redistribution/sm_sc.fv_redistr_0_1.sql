...
create or replace function sm_sc.fv_redistr_0_1
(
  i_array          float[]
)
returns float[]
  ...
  then
    ...
    then 
      ...
      then 
        ...
        return array_fill(1.0 :: float, (select array_agg(array_length(i_array, a_dim) order by a_dim) from generate_series(1, array_ndims(i_array)) tb_a_dim(a_dim)));
      end if;
    else
...
begin
  ...
  then
    ...
    -- --       array_agg
    -- --       (
    -- --         sm_sc.fv_redistr_0_1(i_array[a_cur_y : a_cur_y + i_cnt_per_grp[1] - 1][a_cur_x : a_cur_x + i_cnt_per_grp[2] - 1]) 
    -- --         order by a_cur_x
    -- --       ) as a_slice_x
