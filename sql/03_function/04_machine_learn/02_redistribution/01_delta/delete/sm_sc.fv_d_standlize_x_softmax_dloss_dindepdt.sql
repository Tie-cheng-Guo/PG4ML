...
create or replace function sm_sc.fv_d_standlize_x_softmax_dloss_dindepdt
(
...
begin
  ...
  if i_depdt is null
  then
    i_depdt := sm_sc.fv_standlize_x_softmax(i_indepdt);
  end if;
...
cost 100;
-- -- set search_path to sm_sc;
-- select sm_sc.fv_d_standlize_x_softmax_dloss_dindepdt
--   (
--     sm_sc.fv_standlize_x_softmax(array[array[1, 2, 3, 4, 5], array[5, 4, 3, 2, 1]]),
--     (-` array[array[0.0 :: float, 0.0 :: float, 0.0 :: float, 0.0 :: float, 1.0], array[1.0 :: float, 0.0 :: float, 0.0 :: float, 0.0 :: float, 0.0]] /` sm_sc.fv_standlize_x_softmax(array[array[1, 2, 3, 4, 5], array[5, 4, 3, 2, 1]])),
...
--   sm_sc.fv_d_standlize_x_softmax_dloss_dindepdt
--   (
--     sm_sc.fv_standlize_x_softmax(array[[2.3, 5.1, 8.2, 2.56, 3.33, -1.9], [3.25, 6.4, 6.6, 6.4, -2.65, -4.6], [3.25, 6.4, 6.6, 6.4, -2.65, -4.6]], 3),
