...
create or replace function sm_sc.fv_redistr_l1l2ln
(
  i_array          float[]    ,
  i_n              int
)
...
$$
declare -- here
  v_norm_l1l2ln     float   :=   (|@+| ((@|`  i_array) ^` i_n :: float)) ^ (1.0 :: float/ i_n :: float);
begin
...
cost 100;
-- -- set search_path to sm_sc;
-- select sm_sc.fv_redistr_l1l2ln
--   (
...
begin
  ...
  then
    ...
    -- --       array_agg
    -- --       (
    -- --         sm_sc.fv_redistr_l1l2ln(i_array[a_cur_y : a_cur_y + i_cnt_per_grp[1] - 1][a_cur_x : a_cur_x + i_cnt_per_grp[2] - 1]) 
    -- --         order by a_cur_x
    -- --       ) as a_slice_x
