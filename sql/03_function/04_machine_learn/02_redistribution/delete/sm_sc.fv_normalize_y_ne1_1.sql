...
begin
  ...
  then
    ...
    v_max_y   := sm_sc.fv_aggr_slice_max(i_array, v_y_lens);
    v_ptp_y   := v_max_y -` v_min_y;
    v_2_median_y := v_max_y +` v_min_y;
    return (i_array -` sm_sc.fv_new(v_2_median_y, v_y_lens)) /` sm_sc.fv_new(v_ptp_y /` 2.0 :: float, v_y_lens);
  else
