...
begin
  ...
  then
    ...
    v_ptp_x   := sm_sc.fv_aggr_slice_max(i_array, v_x_lens) -` v_min_x;
      -- sm_sc.fv_ele_replace(sm_sc.fv_aggr_slice_max(i_array, v_x_lens) -` v_min_x, array[0.0 :: float], 'NaN' :: float);      
    return (i_array -` sm_sc.fv_new(v_min_x, v_x_lens)) /` sm_sc.fv_new(v_ptp_x, v_x_lens);
  else
