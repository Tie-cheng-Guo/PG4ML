...
create or replace function sm_sc.fv_conv_de_prod_mx
(
...
declare 
  ...
  v_window_len            int[2] :=    array[array_length(i_window_ex, array_ndims(i_window_ex)), i_window_len_width];
begin
  -- 审计
  ...
  then
    ...
    -- elsif v_window_len[1] <> array_length(i_window_ex, 2)
    -- then 
    --   raise exception 'imperfect window at 1d and window_x at 2d.';
  ...
  then 
    ...
    (
      ...
      (
        select 
          col_a_y,
          sm_sc.fa_mx_concat_x(i_window_ex |**| i_background[col_a_y : col_a_y + v_window_len[1] - 1][col_a_x : col_a_x + v_window_len[2] - 1] order by col_a_x) as a_ret_y
...
cost 100;
-- -- set search_path to sm_sc;
-- select sm_sc.fv_conv_de_prod_mx
--   (
