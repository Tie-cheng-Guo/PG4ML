...
create or replace function sm_sc.fv_pool_none
(
...
$$
declare 
  v_window_len_heigh      int    :=    i_window_len[array_length(i_window_len, 1) - 1]        ;
  v_window_len_width      int    :=    i_window_len[array_length(i_window_len, 1)]            ;
  ...
  v_background_len_heigh_ex     int   :=   coalesce(i_padding[1], 0) + v_background_len_heigh + coalesce(i_padding[2], 0);     --   新背景矩阵高
  v_background_len_width_ex     int   :=   coalesce(i_padding[3], 0) + v_background_len_width + coalesce(i_padding[4], 0);     --   新背景矩阵宽
begin
  -- 审计
  ...
  then
    ...
    elsif array_ndims(i_window_len) <> 1 or array_length(i_window_len, 1) <> 2 and array_length(i_window_len, 1) <> array_ndims(i_background)
    then 
      raise exception 'no method for such i_window length!  Dims: %; len_1: %; len_2: %;', array_dims(i_window_len), v_window_len_heigh, v_window_len_width;
  ...
  then 
    ...
    (
      ...
      (
        select 
          col_a_y,
          sm_sc.fa_mx_concat_x(i_background[col_a_y : col_a_y + v_window_len_heigh - 1][col_a_x : col_a_x + v_window_len_width - 1] order by col_a_x) as a_ret_y
        from generate_series(1, v_background_len_heigh_ex - v_window_len_heigh + i_stride[1], i_stride[1]) tb_a_y(col_a_y)
          , generate_series(1, v_background_len_width_ex - v_window_len_width + i_stride[2], i_stride[2]) tb_a_x(col_a_x)
        group by col_a_y
      )
    ...
    (
      ...
      (
        select 
          ...
          (
            i_background[ : ][col_a_x : col_a_x + v_window_len_heigh - 1][col_a_x3 : col_a_x3 + v_window_len_width - 1] 
            order by col_a_x3
          ) as a_ret_x
        from generate_series(1, v_background_len_heigh - v_window_len_heigh + i_stride[1], i_stride[1]) tb_a_x(col_a_x)
          , generate_series(1, v_background_len_width - v_window_len_width + i_stride[2], i_stride[2]) tb_a_x3(col_a_x3)
        group by col_a_x
      )
    ...
    (
      ...
      (
        select 
          ...
          (
            i_background[ : ][ : ][col_a_x3 : col_a_x3 + v_window_len_heigh - 1][col_a_x4 : col_a_x4 + v_window_len_width - 1]
            order by col_a_x4
          ) as a_ret_x2
        from generate_series(1, v_background_len_heigh - v_window_len_heigh + i_stride[1], i_stride[1]) tb_a_x3(col_a_x3)
          , generate_series(1, v_background_len_width - v_window_len_width + i_stride[2], i_stride[2]) tb_a_x4(col_a_x4)
        group by col_a_x3
      )
