...
create or replace function sm_sc.fv_opr_pool_none_stride_1
(
...
begin
  ...
  then
    -- 审计二维长度
    if array_ndims(i_background) > 4 or array_ndims(i_window_len) > 1
    then 
      raise exception 'no method for such i_background or i_window_len ndims!';
  ...
  then 
    ...
    then 
      ...
      (
        ...
        from generate_series(1, array_length(i_background, 1) - i_window_len[1] + 1) tb_a_y(col_a_y)
      )
      ;
    end if;
  end if;
...
cost 100;
-- -- set search_path to sm_sc;
-- select sm_sc.fv_opr_pool_none_stride_1
--   (
--     array
--        [
