...
create or replace function sm_sc.fv_pool_none_py
(
  i_array              float[]                                              ,
...
$$
  ...
  if v_background.ndim == 2 :    
    ...
    .transpose(0,2,1,3) \
    .reshape(((v_background.shape[-2] - i_window_len[0]) // i_stride[0] + 1) * i_window_len[0], ((v_background.shape[-1] - i_window_len[1]) // i_stride[1] + 1) * i_window_len[1]) \
    .tolist()
    ...
    .transpose(0,1,3,2,4) \
    .reshape(v_background.shape[0], ((v_background.shape[-2] - i_window_len[0]) // i_stride[0] + 1) * i_window_len[0], ((v_background.shape[-1] - i_window_len[1]) // i_stride[1] + 1) * i_window_len[1]) \
    .tolist()
    ...
    .reshape(v_background.shape[0], v_background.shape[1], ((v_background.shape[-2] - i_window_len[0]) // i_stride[0] + 1) * i_window_len[0], ((v_background.shape[-1] - i_window_len[1]) // i_stride[1] + 1) * i_window_len[1]) \
    .tolist()
