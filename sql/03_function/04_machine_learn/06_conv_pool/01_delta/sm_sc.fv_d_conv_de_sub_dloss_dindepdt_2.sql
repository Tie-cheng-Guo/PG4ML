...
-- -- , anyelement
-- );
create or replace function sm_sc.fv_d_conv_de_sub_dloss_dindepdt_2
(
  -- i_window                anyarray           ,
  i_window_len            int[]             ,
...
begin
  ...
  -- then 
  --   i_depdt_var_len := array[array_length(i_depdt_var, array_ndims(i_depdt_var) - 1), array_length(i_depdt_var, array_ndims(i_depdt_var))];
  -- end if;
  -- 
  -- 审计
  ...
  -- -- 审计二维长度
    ...
    elsif array_length(i_window_len, 1) = 3 and i_window_len[1] <> array_length(i_dloss_ddepdt, 1)
      or array_length(i_window_len, 1) = 4 and (i_window_len[1] <> array_length(i_dloss_ddepdt, 1) or i_window_len[2] <> array_length(i_dloss_ddepdt, 2))
    then 
