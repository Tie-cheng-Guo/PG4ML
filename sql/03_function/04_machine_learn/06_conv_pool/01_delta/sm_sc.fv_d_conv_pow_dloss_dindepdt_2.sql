...
--   , anyelement
-- );
create or replace function sm_sc.fv_d_conv_pow_dloss_dindepdt_2
(
...
declare 
  ...
  v_window_len            -- int[2]    :=   array[array_length(i_window, array_ndims(i_window) - 1), array_length(i_window, array_ndims(i_window))];
                          -- alias for i_window_len;
                             int[2]    :=   case 
                                              when i_window is not null 
                                                then array[array_length(i_window, array_ndims(i_window) - 1), array_length(i_window, array_ndims(i_window))]
                                              else i_window_len[array_length(i_window_len, 1) - 1 : array_length(i_window_len, 1)] 
                                            end;
  v_depdt_var_len         -- int[2]    :=   array[array_length(i_depdt_var, array_ndims(i_depdt_var) - 1), array_length(i_depdt_var, array_ndims(i_depdt_var))];
                          -- alias for i_depdt_var_len;
                             int[2]    :=   i_depdt_var_len[array_length(i_depdt_var_len, 1) - 1 : array_length(i_depdt_var_len, 1)];
  v_ret                      i_dloss_ddepdt%type;
begin
  if i_window is not null
  then 
    i_window_len := (select array_agg(array_length(i_window, a_dim) order by a_dim) from generate_series(1, array_ndims(i_window)) tb_a_dim(a_dim));
  end if;  
  ...
  then
    ...
    then 
      raise exception 'no method for such i_background length!  Dims: %; len_1: %; len_2: %;', array_dims(i_background), v_background_len_ex[1], v_background_len_ex[2];
    ...
    elsif v_background_len_ex is null or v_window_len is null or v_depdt_var_len is null or i_dloss_ddepdt is null 
    then 
      raise exception 'v_background_len_ex, v_window_len, v_depdt_var_len, i_dloss_ddepdt should be not null.';
    ...
    elsif v_depdt_var_len is not null and v_depdt_var_len <> v_dloss_ddepdt_len_ex
    then 
      ...
      select 
        ...
        (
          ...
          *` 
          sm_sc.fv_d_pow_2
          (
            case when i_depdt_var is null then i_window end,
            ...
            i_depdt_var[(a_cur_y - 1) * v_window_len[1] + 1 : a_cur_y * v_window_len[1]]
                       [(a_cur_x - 1) * v_window_len[2] + 1 : a_cur_x * v_window_len[2]]
          )
        )
...
--   sm_sc.fv_conv_pow                                         -- i_depdt_var             anyarray 
--   (
...
--   array[[1,2,3,4,5],[3,4,5,6,7],[2,4,6,8,5],[7,5,3,4,8]] :: float[],   -- i_background            anyarray  
--   -- array[4, 5]                   ,                           -- v_background_len_ex        int[2]    
--   null, -- array[[1,4,7],[2,5,8],[3,6,9]] :: float[],                -- i_window                anyarray  
--   array[3, 3]                   ,                           -- v_window_len            int[2]    
--   sm_sc.fv_conv_pow                                         -- i_depdt_var             anyarray 
--   (
--     array[[1,2,3,4,5],[3,4,5,6,7],[2,4,6,8,5],[7,5,3,4,8]] :: float[],
