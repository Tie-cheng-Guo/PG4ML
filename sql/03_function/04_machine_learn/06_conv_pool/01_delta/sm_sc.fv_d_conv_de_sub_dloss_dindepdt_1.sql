...
-- -- , anyelement
-- );
create or replace function sm_sc.fv_d_conv_de_sub_dloss_dindepdt_1
(
  -- i_window                anyarray           ,
  i_window_len            int[]             ,
...
begin
  ...
  then
    ...
    elsif (coalesce(i_padding[1], 0) + v_background_len[1] + coalesce(i_padding[2], 0) - v_window_len[1]) / i_stride[1] + 1 <> v_dloss_ddepdt_len_ex[1] / v_window_len[1]
    then                                                                                                                                         
      raise exception 'imperfect dloss_ddepdt at 1d.';                                                                                           
    elsif (coalesce(i_padding[3], 0) + v_background_len[2] + coalesce(i_padding[4], 0) - v_window_len[2]) / i_stride[2] + 1 <> v_dloss_ddepdt_len_ex[2] / v_window_len[2]
    then 
  ...
  then 
    ...
    (    
      ...
      from generate_series(1, (coalesce(i_padding[1], 0) + v_background_len[1] + coalesce(i_padding[2], 0) - v_window_len[1]) / i_stride[1] + 1) tb_a_cur_y(a_cur_y)
        , generate_series(1, (coalesce(i_padding[3], 0) + v_background_len[2] + coalesce(i_padding[4], 0) - v_window_len[2]) / i_stride[2] + 1) tb_a_cur_x(a_cur_x)
    )
    ;
...
-- (
--   -- null :: float[]               ,                           -- i_window                anyarray  
--   array[3, 3]                   ,                           -- v_window_len            int[2]     -- array[2, 3, 3]
--   -- null :: float[]               ,                           -- i_background            anyarray  
--   array[6, 9]                   ,                           -- i_background_len        int[2]     -- array[2, 6, 9]
--   -- null :: float[]               ,                           -- i_depdt_var             anyarray  
--   -- array[6, 9]                   ,                           -- i_depdt_var_len         int[2]    
--   array               
--   [
