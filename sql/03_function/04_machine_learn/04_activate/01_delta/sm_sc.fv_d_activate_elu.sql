...
create or replace function sm_sc.fv_d_activate_elu
(
  i_indepdt            float[]        ,
  i_asso_value       float    -- 
)
...
begin
  ...
  then
    ...
    loop 
      ...
      loop
        i_indepdt[v_y_cur][v_x_cur] := sm_sc.fv_d_elu_asso(i_indepdt[v_y_cur][v_x_cur], i_asso_value);
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
  ...
  then
    while v_y_cur <= array_length(i_indepdt, 1)
    loop
      i_indepdt[v_y_cur] := sm_sc.fv_d_elu_asso(i_indepdt[v_y_cur], i_asso_value);
      v_y_cur := v_y_cur + 1;
    end loop;
    return i_indepdt;
      ...
      loop
        ...
        loop
          i_indepdt[v_y_cur][v_x_cur][v_x3_cur] = 
            sm_sc.fv_d_elu_asso(i_indepdt[v_y_cur][v_x_cur][v_x3_cur], i_asso_value)
          ;
        end loop;    
      end loop;
    end loop;
        ...
        loop
          ...
          loop
            i_indepdt[v_y_cur][v_x_cur][v_x3_cur][v_x4_cur] = 
              sm_sc.fv_d_elu_asso(i_indepdt[v_y_cur][v_x_cur][v_x3_cur][v_x4_cur], i_asso_value)
            ;
          end loop;
        end loop;    
      end loop;
...
cost 100;
-- select sm_sc.fv_d_activate_elu(array[[1.0 :: float, -2.0], [3.0, 4.0]], 0.3)
-- select sm_sc.fv_d_activate_elu(array[[[1.0 :: float, -2.0], [3.0, 4.0]]], 0.3)
-- select sm_sc.fv_d_activate_elu(array[[[[1.0 :: float, -2.0], [3.0, 4.0]]]], 0.3)
-- select sm_sc.fv_d_activate_elu(array[1.5, -2.5, 3.5], 0.3)
-- select sm_sc.fv_d_activate_elu(array[]::float[], 0.3)
-- select sm_sc.fv_d_activate_elu(array[array[], array []]::float[], 0.3)