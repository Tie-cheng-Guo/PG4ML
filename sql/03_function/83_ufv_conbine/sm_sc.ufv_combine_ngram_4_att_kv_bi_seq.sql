...
create or replace function sm_sc.ufv_combine_ngram_4_att_kv_bi_seq
(
  i_seq_len                             int     -- 要求为偶数，前后缀长度相同
...
begin
  ...
  (
    ...
    , array_fill 
      (
        (i_seq_len / 2 - i_ngram_n + 1) * i_ngram_n
      , array[2]
      )                                                                as node_fn_asso_value
    , 'concat_x'                                                       as node_desc  
    union all                                                          
    select                                                             
      a_ngram_no + 5 :: int                                           as a_tmp_no
    , lower(sm_sc.fv_get_global_seq())                                 as node_no
    , null                                                             as node_type
  ...
  (
    ...
    where tb_a_fore_node.a_tmp_no = 3               -- pool_none
      and tb_a_back_node.a_tmp_no = 1               -- slice_x
    union all 
    select 
      tb_a_fore_node.node_no                         as fore_node_no   
    , 1                                              as path_ord_no
    ...
    from cte_nodes tb_a_fore_node
      , cte_nodes tb_a_back_node
    where tb_a_fore_node.a_tmp_no = 4               -- pool_none
      and tb_a_back_node.a_tmp_no = 2               -- slice_x
    union all 
    select 
      tb_a_fore_node.node_no                         as fore_node_no   
    , 1                                              as path_ord_no
    ...
    from cte_nodes tb_a_fore_node
      , cte_nodes tb_a_back_node
    where tb_a_fore_node.a_tmp_no = 5               -- concat_x
      and tb_a_back_node.a_tmp_no = 3               -- pool_none
    union all 
    select 
      tb_a_fore_node.node_no                         as fore_node_no   
    , 1                                              as path_ord_no
    ...
    from cte_nodes tb_a_fore_node
      , cte_nodes tb_a_back_node
    where tb_a_fore_node.a_tmp_no = 5               -- concat_x
      and tb_a_back_node.a_tmp_no = 4               -- pool_none
    union all 
    select 
      tb_a_fore_node.node_no                         as fore_node_no   
    , 1                                              as path_ord_no
    ...
    from generate_series(1, i_ngram_n) tb_a_ngram_no(a_ngram_no)
      ...
      , cte_nodes tb_a_back_node
    where tb_a_fore_node.a_tmp_no = a_ngram_no + 5  -- sample_x
      and tb_a_back_node.a_tmp_no = 5               -- pool_none
    union all 
    select 
      tb_a_fore_node.node_no                         as fore_node_no   
    ...
    from generate_series(1, i_ngram_n) tb_a_ngram_no(a_ngram_no)
      , cte_nodes tb_a_fore_node
      , cte_nodes tb_a_back_node
    where tb_a_fore_node.a_tmp_no = i_ngram_n + 6   -- concat_x3
      and tb_a_back_node.a_tmp_no = a_ngram_no + 5  -- sample_x
  ),
  -- 准备输入路径
  cte_input_paths as 
  (
...
-- select 
--   sm_sc.ufv_combine_ngram_4_att_kv_bi_seq
--   (
--     40
--   , 64
--   )
...
-- (
--   select
--     -- 以下 jsonb 由调用 sm_sc.ufv_combine_ngram_4_att_kv_bi_seq 获得
--     '
--       {
--         "nodes": [
--           {
