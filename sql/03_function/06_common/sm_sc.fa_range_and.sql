...
create or replace function sm_sc.__fv_range_and_ex
(
...
-- declare
begin
  return coalesce(i_left, '{(,)}') * coalesce(i_right, '{(,)}');
end
$$
language plpgsql stable
parallel safe
cost 100;
-- select sm_sc.__fv_range_and_ex(int8range(1, 2, '[]') :: int8multirange, int8range(5, 6, '[]') :: int8multirange)
-- select sm_sc.__fv_range_and_ex(int8range(1, 2, '[]') :: int8multirange, null)
...
parallel safe
cost 100;
-- select sm_sc.__fv_range_and_ex(int8range(1, 2, '[]'), int8range(5, 6, '[]'))
-- select sm_sc.__fv_range_and_ex(int8range(1, 2, '[]'), '[,]' :: int8range)

-- -----------------------------
-- create or replace aggregate sm_sc.fa_range_and (anymultirange)
drop aggregate if exists sm_sc.fa_range_and(anymultirange);
create aggregate sm_sc.fa_range_and (anymultirange)
(
  sfunc = sm_sc.__fv_range_and_ex,
  stype = anymultirange,
  initcond = '{(,)}',
  parallel = safe
);
...
-- create or replace aggregate sm_sc.fa_range_and (anyrange)
drop aggregate if exists sm_sc.fa_range_and(anyrange);
create aggregate sm_sc.fa_range_and (anyrange)
(
  sfunc = sm_sc.__fv_range_and_ex,
  stype = anyrange,
  initcond = '(,)',
...
create or replace function sm_sc.__fv_range_arr_and
(
  i_left     anyarray    ,
  i_right    anyarray
)
...
begin
  ...
  then 
    ...
    loop
      v_ret[v_y_cur] := sm_sc.__fv_range_and_ex(i_left[v_y_cur], i_right[v_y_cur]);
      v_y_cur := v_y_cur + 1;
    end loop;
    return v_ret;
  ...
  then
    ...
    loop 
      ...
      loop
        v_ret[v_y_cur][v_x_cur] := sm_sc.__fv_range_and_ex(i_left[v_y_cur][v_x_cur], i_right[v_y_cur][v_x_cur]);
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
...
--     array[12.5, 19.1, 5.6]
--   );
-- select sm_sc.__fv_range_arr_and(array[1], array[1,2,3]);
-- select sm_sc.__fv_range_arr_and(array[1], array[array[1,2,3]]);
-- select sm_sc.__fv_range_arr_and(array[array[1]], array[1,2,3]);
-- select sm_sc.__fv_range_arr_and(array[array[1]], array[array[1,2,3]]);
...
begin
  ...
  then
    ...
    loop 
      ...
      loop
        v_ret[v_y_cur][v_x_cur] := sm_sc.__fv_range_and_ex(i_left[v_y_cur][v_x_cur], i_right);
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
  ...
  then
    ...
    while v_y_cur <= array_length(i_left, 1)
    loop
      v_ret[v_y_cur] := sm_sc.__fv_range_and_ex(i_left[v_y_cur], i_right);
      v_y_cur := v_y_cur + 1;
    end loop;
    return v_ret;
...
parallel safe
cost 100;
-- select sm_sc.__fv_range_arr_and(array[array[12.3, 25.1], array[2.56, 3.25]], 8.8)
-- select sm_sc.__fv_range_arr_and(array[12.3, 25.1, 2.56, 3.25], 8.8)
-- select sm_sc.__fv_range_arr_and(array[]::float[], 8.8::float)
-- select sm_sc.__fv_range_arr_and(array[array[], array []]::float[], 8.8::float)
-- select 
--   sm_sc.__fv_range_arr_and
--   (
--     array[[[1,2,3],[1.2,2.3,3.4]],[[0.5,0.7,0.8],[0.3,0.4,0.7]]]
...
begin
  ...
  then
    ...
    loop 
      ...
      loop
        v_ret[v_y_cur][v_x_cur] := sm_sc.__fv_range_and_ex(i_left, i_right[v_y_cur][v_x_cur]);
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
  ...
  then
    ...
    while v_y_cur <= array_length(i_right, 1)
    loop
      v_ret[v_y_cur] := sm_sc.__fv_range_and_ex(i_left, i_right[v_y_cur]);
      v_y_cur := v_y_cur + 1;
    end loop;
    return v_ret;
...
parallel safe
cost 100;
-- select sm_sc.__fv_range_arr_and(8.8, array[array[12.3, 25.1], array[2.56, 3.25]])
-- select sm_sc.__fv_range_arr_and(8.8, array[12.3, 25.1, 2.56, 3.25])
-- select sm_sc.__fv_range_arr_and(8.8::float, array[]::float[] )
-- select sm_sc.__fv_range_arr_and(8.8::float, array[array[], array []]::float[])
-- select 
--   sm_sc.__fv_range_arr_and
--   (
--     2.2
...
-- ---------------------------------------------------------------------
-- create or replace aggregate sm_sc.fa_range_and (anyarray)
drop aggregate if exists sm_sc.fa_range_and(anyarray);
create aggregate sm_sc.fa_range_and (anyarray)
(
  sfunc = sm_sc.__fv_range_arr_and,
  stype = anyarray,
  initcond = '{}',
  parallel = safe
);
