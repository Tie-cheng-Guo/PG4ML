...
create or replace procedure sm_sc.prc_nn_prepare_p
(
...
$$
declare -- here
  v_model_code_6           char(6)              := (select model_code_6 from sm_sc.tb_classify_task where work_no = i_work_no);
...
-- -- raise notice 'input_01 v_input_asso_value: %', array_dims(v_input_asso_value);
  ...
  -- 审计 训练集同一输入节点每条数据的规格同一
    ...
    (
      ...
      having (count(distinct array_dims(i_indepdt_01)) > 1
              ...
              or count(distinct array_dims(i_depdt_01)) > 1
  ...
  -- 非 weight 的 00_const 节点的 p_node_depdt 初始化审计工作
  elsif exists 
    (
      select  
  ...
  then 
    raise exception 'there should be not any null p_node_depdt or unmatch length configured by node_fn_asso_value at 00_const.';
...
raise notice 'check data len begin!!!!!!';
  ...
  loop
    ...
    (
      select 
        ...
        case when tb_a_fore.node_fn_type not like '06_aggr_mx_%' then tb_a_back_p3.node_depdt_len end as a_p3_len,
      ...
      left join sm_sc.tb_nn_path tb_a_path_p3
        on tb_a_path_p3.work_no = i_work_no   -- 2021082501
          and tb_a_path_p3.path_ord_no = 3 -- -- 如果与 前向节点的 path_ord_no 冲突，使用 00_none 算子隔离。
          and tb_a_path_p3.fore_node_no = tb_a_fore.node_no
      left join sm_sc.tb_nn_node tb_a_back_p3
        on tb_a_back_p3.work_no = i_work_no   -- 2021082501
          and tb_a_back_p3.node_no = tb_a_path_p3.back_node_no
  ...
  if exists
    (
      with 
      cte_dataset_limit_1 as 
      (
        select 
          (select array_agg(array_length(i_depdt_01, a_no_depdt_01) order by a_no_depdt_01) from generate_series(1, array_ndims(i_depdt_01)) tb_a_depdt_01(a_no_depdt_01)) as a_depdt_01_len
        , (select array_agg(array_length(i_depdt_02, a_no_depdt_02) order by a_no_depdt_02) from generate_series(1, array_ndims(i_depdt_02)) tb_a_depdt_02(a_no_depdt_02)) as a_depdt_02_len
        , (select array_agg(array_length(i_depdt_03, a_no_depdt_03) order by a_no_depdt_03) from generate_series(1, array_ndims(i_depdt_03)) tb_a_depdt_03(a_no_depdt_03)) as a_depdt_03_len
        , (select array_agg(array_length(i_depdt_04, a_no_depdt_04) order by a_no_depdt_04) from generate_series(1, array_ndims(i_depdt_04)) tb_a_depdt_04(a_no_depdt_04)) as a_depdt_04_len
        , (select array_agg(array_length(i_indepdt_01, a_no_indepdt_01) order by a_no_indepdt_01) from generate_series(1, array_ndims(i_indepdt_01)) tb_a_indepdt_01(a_no_indepdt_01)) as a_indepdt_01_len
        , (select array_agg(array_length(i_indepdt_02, a_no_indepdt_02) order by a_no_indepdt_02) from generate_series(1, array_ndims(i_indepdt_02)) tb_a_indepdt_02(a_no_indepdt_02)) as a_indepdt_02_len
        , (select array_agg(array_length(i_indepdt_03, a_no_indepdt_03) order by a_no_indepdt_03) from generate_series(1, array_ndims(i_indepdt_03)) tb_a_indepdt_03(a_no_indepdt_03)) as a_indepdt_03_len
        , (select array_agg(array_length(i_indepdt_04, a_no_indepdt_04) order by a_no_indepdt_04) from generate_series(1, array_ndims(i_indepdt_04)) tb_a_indepdt_04(a_no_indepdt_04)) as a_indepdt_04_len
      ...
      select 
      from 
        sm_sc.tb_nn_node tb_a_node
      , cte_dataset_limit_1
      where tb_a_node.work_no = i_work_no
        and 
        (
           tb_a_node.node_type = 'output_01' and node_depdt_len[2 : ] <> a_depdt_01_len
        or tb_a_node.node_type = 'output_02' and node_depdt_len[2 : ] <> a_depdt_02_len
        or tb_a_node.node_type = 'output_03' and node_depdt_len[2 : ] <> a_depdt_03_len
        or tb_a_node.node_type = 'output_04' and node_depdt_len[2 : ] <> a_depdt_04_len
        or tb_a_node.node_type = 'input_01' and node_depdt_len[2 : ] <> a_indepdt_01_len
        or tb_a_node.node_type = 'input_02' and node_depdt_len[2 : ] <> a_indepdt_02_len
        or tb_a_node.node_type = 'input_03' and node_depdt_len[2 : ] <> a_indepdt_03_len
        or tb_a_node.node_type = 'input_04' and node_depdt_len[2 : ] <> a_indepdt_04_len
        )
    )
  then 
    raise exception 'unmatched input or output len with dataset in tb_nn_train_input_buff';
  end if;
