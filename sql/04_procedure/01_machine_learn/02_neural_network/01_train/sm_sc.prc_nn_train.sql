...
-- raise notice 'debug 001. step begin: v_cur_learn_cnt: %; at %;', v_cur_learn_cnt, date_trunc('milliseconds', now());
    ...
    -- 填充 dropout 节点的因变量
    update sm_sc.tb_nn_node
    set node_depdt_vals = array_fill(dropout_depdt_val, node_depdt_len)
    where work_no = i_work_no
      and is_dropout
    ;
    ...
    then 
      ...
      update sm_sc.tb_nn_node tb_a_tar
      set 
        node_depdt_vals = case tb_a_tar.node_type when 'input_01' then a_x_vals_01 when 'input_02' then a_x_vals_02 when 'input_03' then a_x_vals_03 when 'input_04' then a_x_vals_04 end  
     ...
     -- , (sm_sc.fa_mx_or(case when tb_a_path.path_ord_no = 3 then array[tb_a_back.is_back_node] else array[false] end))[1] as a_is_bi_opr_input_3rd_back
      ...
      (
        ...
        returning 
          ...
          -- 寄存、传递若干算子的因变量，对 ddepdt_dindepdt 是必要的。
        , case when tb_a_tar_depdt.node_fn_type = any(v_arr_fn_depdt_delta) then tb_a_tar_depdt.node_depdt_vals end as a_node_depdt_vals
      )
--       ,
      -- 前向传播同步求导 ddepdt_dindepdt
          ...
          ddepdt_dindepdt =
            ...
            , case 
                when tb_a_tar_path.path_ord_no = 1 and substr(tb_a_dic.enum_value, 1, 1) = '1'
                  then tb_a_indepdt_fore.a_bi_opr_input_1st 
                when tb_a_tar_path.path_ord_no = 2 and substr(tb_a_dic.enum_value, 2, 1) = '1'
                  then tb_a_indepdt_fore.a_bi_opr_input_2nd
             -- when tb_a_tar_path.path_ord_no = 3 
             --   then tb_a_indepdt_fore.a_bi_opr_input_3rd
              end 
            ...
            , case 
                when tb_a_tar_path.path_ord_no = 1 and substr(tb_a_dic.enum_value, 5, 1) = '1'
                  ...
                  then tb_a_depdt_val.a_node_depdt_vals
              end 
            , case 
        ...
        set 
          ddepdt_dindepdt =
            case tb_a_fore.node_fn_type
              when '06_aggr_mx_sum'
                then sm_sc.fv_d_mx_sum(tb_a_indepdt_fore.a_opr_depdt_len)
              when '06_aggr_mx_prod'
                then sm_sc.fv_d_mx_prod(tb_a_back.node_depdt_vals, tb_a_fore.node_depdt_vals)
              when '06_aggr_mx_avg'
                then sm_sc.fv_d_mx_avg(tb_a_fore.node_fn_asso_value[1] :: int, tb_a_indepdt_fore.a_opr_depdt_len)
              when '06_aggr_mx_max'
                then sm_sc.fv_d_mx_max(tb_a_back.node_depdt_vals, tb_a_fore.node_depdt_vals)
              when '06_aggr_mx_min'
                then sm_sc.fv_d_mx_min(tb_a_back.node_depdt_vals, tb_a_fore.node_depdt_vals)
            end
        from cte_indepdt tb_a_indepdt_fore
...
-- raise notice 'debug 003.1. v_depdt_01: %', array_dims(v_depdt_01);
    ...
    v_cur_loss = 
      ...
      , (select node_depdt_vals from sm_sc.tb_nn_node where work_no = i_work_no and node_type = 'output_01')
      , v_depdt_01
      , (select node_depdt_vals from sm_sc.tb_nn_node where work_no = i_work_no and node_type = 'output_02')
      , v_depdt_02
      , (select node_depdt_vals from sm_sc.tb_nn_node where work_no = i_work_no and node_type = 'output_03')
      , v_depdt_03
      , (select node_depdt_vals from sm_sc.tb_nn_node where work_no = i_work_no and node_type = 'output_04')
      , v_depdt_04
      ) 
    ;
          ...
          (
            ...
            , case 
                when (tb_a_path.path_ord_no = 1 or tb_a_fore.node_fn_type like '06_aggr_%')
                  ...
                  then tb_a_back.node_depdt_vals 
              end
            ...
            , case 
                when (tb_a_path.path_ord_no = 1 or tb_a_fore.node_fn_type like '06_aggr_%')
                  ...
                  then tb_a_back_co.node_depdt_vals
              end
            ...
            , case 
                when (tb_a_path.path_ord_no = 1 or tb_a_fore.node_fn_type like '06_aggr_%')
                  ...
                  then tb_a_fore.node_depdt_vals
              end
            , tb_a_fore.node_dloss_ddepdt
            , case 
                when (tb_a_path.path_ord_no = 1 or tb_a_fore.node_fn_type like '06_aggr_%')
...
-- (
        ...
        set 
          node_dloss_ddepdt = t_sour.a_dloss_dindepdt,
          learn_cnt_back = v_cur_learn_cnt + 1 -- 强制对齐训练次数，避免路径计算混乱    -- t_tar.learn_cnt_back + 1
...
-- from cte_upd_node
      ...
      update sm_sc.tb_nn_node t_tar
      set 
        node_dloss_ddepdt = sm_sc.fv_aggr_chunk_sum(node_dloss_ddepdt, node_depdt_len)
      where work_no = i_work_no   -- 2021082501
        and nn_depth_no = v_cur_nn_depth - 1
        and cardinality(node_dloss_ddepdt) / cardinality(node_depdt_vals) > 1
        and cardinality(node_dloss_ddepdt) % cardinality(node_depdt_vals) = 0
      ; 
      commit;
...
-- raise notice 'debug 007. step back end: v_cur_learn_cnt: %; v_cur_node_nos: %; v_cur_nn_depth: %; at: %;', v_cur_learn_cnt, v_cur_node_nos, v_cur_nn_depth, date_trunc('milliseconds', now());
    ...
    then  
      ...
      set 
        node_depdt_vals = tar.node_depdt_vals -` i_l1
      where work_no = i_work_no   -- 2021082501
    ...
    then  
      update sm_sc.tb_nn_node tar
      set 
        node_depdt_vals = tar.node_depdt_vals *` (1.0 - i_l2)
      where work_no = i_work_no   -- 2021082501
        and node_type = 'weight'
      ;
      commit;
    ...
    then 
      update sm_sc.tb_nn_node tar
      set 
        -- node_depdt_vals = tar.node_depdt_vals -` (i_learn_rate *` tar.node_dloss_ddepdt)
        node_depdt_vals = 
          tar.node_depdt_vals 
          -` 
          (
      ...
      set 
        -- node_depdt_vals = tar.node_depdt_vals -` (i_learn_rate *` tar.node_dloss_ddepdt)
        cost_delta_l1 = (v_beta_l1 *` coalesce(case when array_length(cost_delta_l1, 1) = array_length(tar.node_dloss_ddepdt, 1) then cost_delta_l1 else null end, tar.node_dloss_ddepdt)) +` ((1.0 :: float - v_beta_l1) *` tar.node_dloss_ddepdt),
        cost_delta_l2 = (v_beta_l2 *` coalesce(case when array_length(cost_delta_l2, 1) = array_length(tar.node_dloss_ddepdt, 1) then cost_delta_l2 else null end, (tar.node_dloss_ddepdt ^` 2.0 :: float))) +` ((1.0 :: float - v_beta_l2) *` (tar.node_dloss_ddepdt ^` 2.0 :: float))
      where work_no = i_work_no   -- 2021082501
        and node_type = 'weight'
      ;
      commit;
    ...
    then 
      ...
      select 
        ...
        case when tb_a_sour.node_type = 'weight' or tb_a_sour.node_fn_type = '00_const' then tb_a_sour.node_depdt_vals end as node_depdt_vals                 
      from sm_sc.tb_nn_node tb_a_sour
      where tb_a_sour.work_no = i_work_no
      ...
      update set
        ...
        node_depdt_vals                 = case when EXCLUDED.node_type = 'weight' or EXCLUDED.node_fn_type = '00_const' then EXCLUDED.node_depdt_vals end
      ;    
      commit;
...
-- select node_no, node_fn_type, learn_cnt_fore, learn_cnt_back, tb_a.node_depdt_vals, (tb_a.reg_w).m_vals  from sm_sc.tb_nn_node tb_a where work_no = pgv_get('vars', 'this_work_no_02', NULL::bigint)
-- select 
--   sm_sc.ft_nn_in_out
--   (
--     pgv_get('vars', 'this_work_no_02', NULL::bigint), 
