...
(
     select trim('l_w_r_h_chunk_pos__1d  '), trim('入参一高度规格，也即列的高度     '), trim('81_prod_mx_chunk       '),  1, numrange(1.0, 1.0, '[]')
)
select 
...
(
  select trim('81_prod_mx_chunk      '),         trim('sm_sc.ufv_prod_mx_chunk          '),            '3_p'
)
select 
...
-- (
--      select trim('dldi_query_from_col      '), trim('sm_sc.ufv_d_query_by_width_row_idx_from_col_dloss_dindepdt_1      '),trim('81_prod_mx_chunk      ')
-- )
-- select 
...
-- (
--   select trim('81_prod_mx_chunk      '),         trim('00100010'),            '1'
-- )
-- select 
