...
-- (
--      select trim('p1_len_row__1d  '), trim('入参一宽度规格，也即行的宽度     '), trim('81_query_from_row       '),  1, numrange(1.0, 1.0, '[]')
-- )
-- select 
...
(
  select trim('81_query_from_row      '),         trim('sm_sc.ufv_query_by_heigh_col_idx_from_row          '),            '2_p'
)
select 
...
(
     select trim('dldi_query_from_row      '), trim('sm_sc.ufv_d_query_by_heigh_col_idx_from_row_dloss_dindepdt_1      '),trim('81_query_from_row      ')
)
select 
...
--   第五位控制：第一目求导是否需要因变量
--   第六位控制：第二目求导是否需要因变量
--   第七位控制：第一目求导是否用到自变量高宽规格
--   第八位控制：第二目求导是否用到自变量高宽规格
(
  select trim('81_query_from_row      '),         trim('00100010'),            '1'
)
select 
