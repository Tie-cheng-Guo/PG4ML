...
-- (
--      select trim('p1_len_col__1d  '), trim('入参一高度规格，也即列的高度     '), trim('81_query_from_col       '),  1, numrange(1.0, 1.0, '[]')
-- )
-- select 
...
(
  select trim('81_query_from_col      '),         trim('sm_sc.ufv_query_by_width_row_idx_from_col          '),            '2_p'
)
select 
...
(
     select trim('dldi_query_from_col      '), trim('sm_sc.ufv_d_query_by_width_row_idx_from_col_dloss_dindepdt_1      '),trim('81_query_from_col      ')
)
select 
...
(
  select trim('81_query_from_col      '),         trim('00100010'),            '1'
)
select 
