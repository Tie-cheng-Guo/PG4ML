...
-- drop function if exists floor(sm_sc.typ_l_complex, int);
...
begin
  return 
    (
      case 
        when i_left.m_re = round(i_left.m_re, i_right) 
          then i_left.m_re
        else round(i_left.m_re - v_balan, i_right) 
      end,
      case 
        when i_left.m_im = round(i_left.m_im, i_right) 
          then i_left.m_im
        else round(i_left.m_im - v_balan, i_right) 
      end
    )::sm_sc.typ_l_complex;
end
$$
...
-- -------------------------------------------------------------------------------------
-- set search_path to schm_cpx;
-- drop function if exists floor(sm_sc.typ_l_complex);
create or replace function floor
(
