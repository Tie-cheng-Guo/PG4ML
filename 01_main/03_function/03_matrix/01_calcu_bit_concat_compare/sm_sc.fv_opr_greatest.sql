...
create or replace function sm_sc.fv_opr_greatest
(
  i_left     anyarray    ,
  i_right    anyarray
)
...
begin
  ...
  then 
    return sm_sc.fv_opr_greatest(i_left[1], i_right);
  -- greatest([], [.])
  elsif array_ndims(i_right) = 1 and array_length(i_right, 1) = 1
  then 
    return sm_sc.fv_opr_greatest(i_left, i_right[1]);

  -- greatest([[.]], [])
  elsif array_length(i_left, 2) = 1 and array_length(i_left, 1) = 1
  then 
    return sm_sc.fv_opr_greatest(i_left[1][1], i_right);
  -- greatest([], [[.]])
  elsif array_length(i_right, 2) = 1 and array_length(i_right, 1) = 1
  then 
    return sm_sc.fv_opr_greatest(i_left, i_right[1][1]);
  ...
  then 
    ...
    loop
      i_left[v_y_cur] := greatest(i_left[v_y_cur], i_right[v_y_cur]);
      v_y_cur := v_y_cur + 1;
    end loop;
    return i_left;
  ...
  then
    ...
    loop 
      ...
      loop
        i_left[v_y_cur][v_x_cur] := greatest(i_left[v_y_cur][v_x_cur], i_right[v_y_cur][v_x_cur]);
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
    return i_left;
  -- 横向广播，i_left 需要延拓 greatest([][1], [][])
  ...
  then
    ...
    loop 
      ...
      while v_x_cur <= array_length(i_right, 2)
      loop
        i_right[v_y_cur][v_x_cur] := greatest(i_left[v_y_cur][1], i_right[v_y_cur][v_x_cur]);
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
    return i_right;
  -- 横向广播，i_right 需要延拓 greatest([][], [][1])
  ...
  then
    ...
    loop 
      ...
      loop
        i_left[v_y_cur][v_x_cur] := greatest(i_left[v_y_cur][v_x_cur], i_right[v_y_cur][1]);
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
    return i_left;
  -- 纵向广播 i_left 需要延拓 greatest([1][], [][])
  ...
  then
    ...
    loop 
      ...
      while v_x_cur <= array_length(i_right, 2)
      loop
        i_right[v_y_cur][v_x_cur] := greatest(i_left[1][v_x_cur], i_right[v_y_cur][v_x_cur]);
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
    return i_right;
  -- 纵向广播 i_right 需要延拓 greatest([][], [1][])
  ...
  then
    ...
    loop 
      ...
      while v_x_cur <= array_length(i_left, 2)
      loop
        i_left[v_y_cur][v_x_cur] := greatest(i_left[v_y_cur][v_x_cur], i_right[1][v_x_cur]);
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
...
--   );
-- -- select sm_sc.fv_opr_greatest    -- 设计不支持一维二维混合运算
-- --   (
-- --     array[32.5, -9.1],
-- --     array[array[-12.3, 12.3], array[-45.6, 45.6]]
-- --   );
...
--     array[2.1, 3.2]::float[]
--   );
-- select sm_sc.fv_opr_greatest(array[2.1], array[1.2, 2.1, 3.2]);
-- select sm_sc.fv_opr_greatest(array[2.1], array[array[1.2, 2.1, 3.2]]);
-- select sm_sc.fv_opr_greatest(array[array[2.1]], array[1.2, 2.1, 3.2]);
-- select sm_sc.fv_opr_greatest(array[array[2.1]], array[array[1.2, 2.1, 3.2]]);
...
declare -- here
  ...
  v_y_cur   int  := 1  ;
begin
  -- greatest(null :: float[][], float = null :: float[][])
  ...
  then
    ...
    loop 
      ...
      while v_x_cur <= array_length(i_left, 2)
      loop
        i_left[v_y_cur][v_x_cur] := greatest(i_left[v_y_cur][v_x_cur], i_right);
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
  ...
  then
    while v_y_cur <= array_length(i_left, 1)
    loop
      i_left[v_y_cur] := greatest(i_left[v_y_cur], i_right);
      v_y_cur := v_y_cur + 1;
    end loop;
    return i_left;
...
parallel safe
cost 100;
-- select sm_sc.fv_opr_greatest(array[array[12.3, 25.1], array[2.56, 3.25]], 8.8)
-- select sm_sc.fv_opr_greatest(array[]::float[], 8.8)
-- select sm_sc.fv_opr_greatest(array[]::float[], null)
-- select sm_sc.fv_opr_greatest(array[1.2, 2.3]::float[], 8.8)
-- select sm_sc.fv_opr_greatest(array[array[], array []]::float[], 8.8)
-- select sm_sc.fv_opr_greatest(array[1.2, 2.3]::float[], null::float)
...
begin
  ...
  then
    ...
    loop 
      ...
      while v_x_cur <= array_length(i_right, 2)
      loop
        i_right[v_y_cur][v_x_cur] := greatest(i_left, i_right[v_y_cur][v_x_cur]);
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
  ...
  then
    while v_y_cur <= array_length(i_right, 1)
    loop
      i_right[v_y_cur] := greatest(i_left, i_right[v_y_cur]);
      v_y_cur := v_y_cur + 1;
    end loop;
    return i_right;
...
parallel safe
cost 100;
-- select sm_sc.fv_opr_greatest(8.8, array[array[12.3, 25.1], array[2.56, 3.25]])
-- select sm_sc.fv_opr_greatest(8.8, array[]::float[] )
-- select sm_sc.fv_opr_greatest(null, array[]::float[])
-- select sm_sc.fv_opr_greatest(8.8, array[1.2, 2.3]::float[] )
-- select sm_sc.fv_opr_greatest(8.8, array[array[], array []]::float[])
-- select sm_sc.fv_opr_greatest(null::float, array[1.2, 2.3]::float[] )
