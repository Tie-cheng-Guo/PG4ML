...
create or replace function sm_sc.fv_opr_and
(
  i_left     boolean[]    ,
  i_right    boolean[]
)
...
begin
  ...
  then 
    ...
    loop
      i_left[v_y_cur] := i_left[v_y_cur] and i_right[v_y_cur];
      v_y_cur := v_y_cur + 1;
    end loop;
    return i_left;
  ...
  then
    ...
    loop 
      ...
      loop
        i_left[v_y_cur][v_x_cur] := i_left[v_y_cur][v_x_cur] and i_right[v_y_cur][v_x_cur];
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
    return i_left;
  -- 横向广播，i_left 需要延拓 [][1] and [][]
  ...
  then
    ...
    loop 
      ...
      while v_x_cur <= array_length(i_right, 2)
      loop
        i_right[v_y_cur][v_x_cur] := i_left[v_y_cur][1] and i_right[v_y_cur][v_x_cur];
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
    return i_right;
  -- 横向广播，i_right 需要延拓 [][] and [][1]
  ...
  then
    ...
    loop 
      ...
      loop
        i_left[v_y_cur][v_x_cur] := i_left[v_y_cur][v_x_cur] and i_right[v_y_cur][1];
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
    return i_left;
  -- 纵向广播 i_left 需要延拓 [1][] and [][]
  ...
  then
    ...
    loop 
      ...
      while v_x_cur <= array_length(i_right, 2)
      loop
        i_right[v_y_cur][v_x_cur] := i_left[1][v_x_cur] and i_right[v_y_cur][v_x_cur];
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
    return i_right;
  -- 纵向广播 i_right 需要延拓 [][] and [1][]
  ...
  then
    ...
    loop 
      ...
      while v_x_cur <= array_length(i_left, 2)
      loop
        i_left[v_y_cur][v_x_cur] := i_left[v_y_cur][v_x_cur] and i_right[1][v_x_cur];
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
...
--     array[true, false]
--   );
-- select sm_sc.fv_opr_and(array[true], array[true, false]);
-- select sm_sc.fv_opr_and(array[false], array[array[true, false]]);
-- select sm_sc.fv_opr_and(array[array[true]], array[true, false]);
-- select sm_sc.fv_opr_and(array[array[false]], array[array[true, false]]);
...
declare -- here
  ...
  v_y_cur   int  := 1  ;
begin
  -- null :: float[][] and float = null :: float[][]
  ...
  then
    ...
    loop 
      ...
      while v_x_cur <= array_length(i_left, 2)
      loop
        i_left[v_y_cur][v_x_cur] := i_left[v_y_cur][v_x_cur] and i_right;
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
  ...
  then
    while v_y_cur <= array_length(i_left, 1)
    loop
      i_left[v_y_cur] := i_left[v_y_cur] and i_right;
      v_y_cur := v_y_cur + 1;
    end loop;
    return i_left;
...
parallel safe
cost 100;
-- select sm_sc.fv_opr_and(array[array[true, false], array[false, true]], false)
-- select sm_sc.fv_opr_and(array[true, false], false)
-- select sm_sc.fv_opr_and(array[]::boolean[], true)
-- select sm_sc.fv_opr_and(array[array[], array []]::boolean[], false)
...
begin
  ...
  then
    ...
    loop 
      ...
      while v_x_cur <= array_length(i_right, 2)
      loop
        i_right[v_y_cur][v_x_cur] := i_left and i_right[v_y_cur][v_x_cur];
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
  ...
  then
    while v_y_cur <= array_length(i_right, 1)
    loop
      i_right[v_y_cur] := i_left and i_right[v_y_cur];
      v_y_cur := v_y_cur + 1;
    end loop;
    return i_right;
...
parallel safe
cost 100;
-- select sm_sc.fv_opr_and(false, array[array[false, true], array[true, false]])
-- select sm_sc.fv_opr_and(false, array[true, false])
-- select sm_sc.fv_opr_and(false, array[]::boolean[] )
-- select sm_sc.fv_opr_and(true, array[array[], array []]::boolean[])
...
--     array[B'010', B'011']
--   );
-- select sm_sc.fv_opr_and(array[B'101'], array[B'101', B'011']);
-- select sm_sc.fv_opr_and(array[B'101'], array[array[B'101', B'011']]);
-- select sm_sc.fv_opr_and(array[array[B'101']], array[B'101', B'011']);
-- select sm_sc.fv_opr_and(array[array[B'101']], array[array[B'101', B'011']]);
...
parallel safe
cost 100;
-- select sm_sc.fv_opr_and(array[array[B'010', B'011'], array[B'011', B'010']], B'011')
-- select sm_sc.fv_opr_and(array[B'010011', B'011010'], B'011101')
-- select sm_sc.fv_opr_and(array[]::varbit[], B'010')
-- select sm_sc.fv_opr_and(array[array[], array []]::varbit[], B'011')
...
parallel safe
cost 100;
-- select sm_sc.fv_opr_and(B'110', array[array[B'011', B'010'], array[B'101', B'110']])
-- select sm_sc.fv_opr_and(B'011101', array[B'010011', B'011010'])
-- select sm_sc.fv_opr_and(B'011', array[]::varbit[] )
-- select sm_sc.fv_opr_and(B'010', array[array[], array []]::varbit[])
