...
begin
  ...
  then 
    return sm_sc.fv_opr_shift_left(i_left[1], i_right);
  -- [] << [.]
  elsif array_ndims(i_right) = 1 and array_length(i_right, 1) = 1
  then 
    return sm_sc.fv_opr_shift_left(i_left, i_right[1]);

  -- [[.]] << []
  elsif array_length(i_left, 2) = 1 and array_length(i_left, 1) = 1
  then 
    return sm_sc.fv_opr_shift_left(i_left[1][1], i_right);
  -- [] << [[.]]
  elsif array_length(i_right, 2) = 1 and array_length(i_right, 1) = 1
  then 
    return sm_sc.fv_opr_shift_left(i_left, i_right[1][1]);
