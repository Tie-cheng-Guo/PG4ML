...
begin
  ...
  then 
    return sm_sc.fv_opr_least(i_left[1], i_right);
  -- least([], [.])
  elsif array_ndims(i_right) = 1 and array_length(i_right, 1) = 1
  then 
    return sm_sc.fv_opr_least(i_left, i_right[1]);

  -- least([[.]], [])
  elsif array_length(i_left, 2) = 1 and array_length(i_left, 1) = 1
  then 
    return sm_sc.fv_opr_least(i_left[1][1], i_right);
  -- least([], [[.]])
  elsif array_length(i_right, 2) = 1 and array_length(i_right, 1) = 1
  then 
    return sm_sc.fv_opr_least(i_left, i_right[1][1]);
  ...
  then
    ...
    loop 
      ...
      while v_x_cur <= array_length(i_right, 2)
      loop
        i_right[v_y_cur][v_x_cur] := least(i_left[v_y_cur][1], i_right[v_y_cur][v_x_cur]);
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
    return i_right;
  -- 横向广播，i_right 需要延拓 least([][], [][1])
  ...
  then
    ...
    loop 
      ...
      loop
        i_left[v_y_cur][v_x_cur] := least(i_left[v_y_cur][v_x_cur], i_right[v_y_cur][1]);
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
    return i_left;
  -- 纵向广播 i_left 需要延拓 least([1][], [][])
  ...
  then
    ...
    loop 
      ...
      while v_x_cur <= array_length(i_right, 2)
      loop
        i_right[v_y_cur][v_x_cur] := least(i_left[1][v_x_cur], i_right[v_y_cur][v_x_cur]);
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
    return i_right;
  -- 纵向广播 i_right 需要延拓 least([][], [1][])
  ...
  then
    ...
    loop 
      ...
      while v_x_cur <= array_length(i_left, 2)
      loop
        i_left[v_y_cur][v_x_cur] := least(i_left[v_y_cur][v_x_cur], i_right[1][v_x_cur]);
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
...
-- select sm_sc.fv_opr_least(array[]::float[], 8.8)
-- select sm_sc.fv_opr_least(array[]::float[], null)
-- select sm_sc.fv_opr_least(array[1.2, 2.3]::float[], 8.8)
-- select sm_sc.fv_opr_least(array[array[], array []]::float[], 8.8)
...
-- select sm_sc.fv_opr_least(8.8, array[]::float[] )
-- select sm_sc.fv_opr_least(null, array[]::float[])
-- select sm_sc.fv_opr_least(8.8, array[1.2, 2.3]::float[] )
-- select sm_sc.fv_opr_least(8.8, array[array[], array []]::float[])
