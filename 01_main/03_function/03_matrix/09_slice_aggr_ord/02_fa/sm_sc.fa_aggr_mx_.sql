...
-- ---------------------
-- create or replace function sm_sc.fv_arr_float_add_ex
drop function if exists sm_sc.fv_arr_float_add_ex;
create function sm_sc.fv_arr_float_add_ex
(
...
parallel safe
cost 100;
-- select sm_sc.fv_arr_float_add_ex(row(array[array[1.2, 1.3], array[2.3, 2.5]], 2), array[array[1.0 :: float, 1.0], array[2.0 :: float, 2.0]])

-- ---------------------
-- create or replace function sm_sc.fv_mx_avg_final
drop function if exists sm_sc.fv_mx_avg_final;
create function sm_sc.fv_mx_avg_final
(
...
parallel safe
cost 100;
-- select sm_sc.fv_mx_avg_final(row(array[array[1.2, 1.3], array[2.3, 2.5]], 2))
...
(
  sfunc = sm_sc.fv_arr_float_add_ex,
  stype = sm_sc.__typ_arr_float_ex,
  initcond = '("{{0}}",0)',
  finalfunc = sm_sc.fv_mx_avg_final,
  parallel = safe
);
...
-- ---------------------
-- create or replace function sm_sc.fv_arr_cpx_add_ex
drop function if exists sm_sc.fv_arr_cpx_add_ex;
create function sm_sc.fv_arr_cpx_add_ex
(
...
parallel safe
cost 100;
-- select sm_sc.fv_arr_cpx_add_ex(row(array[array[(1.2, 1.6)::sm_sc.typ_l_complex, (1.3, 1.9)::sm_sc.typ_l_complex], array[(2.3, -2.1)::sm_sc.typ_l_complex, (2.5, -2.6)::sm_sc.typ_l_complex]], 2), array[array[(1.2, 1.6)::sm_sc.typ_l_complex, (1.3, 1.9)::sm_sc.typ_l_complex], array[(2.3, -2.1)::sm_sc.typ_l_complex, (2.5, -2.6)::sm_sc.typ_l_complex]])

-- ---------------------
-- create or replace function sm_sc.fv_mx_avg_final_cpx
drop function if exists sm_sc.fv_mx_avg_final_cpx;
create function sm_sc.fv_mx_avg_final_cpx
(
...
parallel safe
cost 100;
-- select sm_sc.fv_mx_avg_final_cpx(row(array[array[(1.2, 1.6)::sm_sc.typ_l_complex, (1.3, 1.9)::sm_sc.typ_l_complex], array[(2.3, -2.1)::sm_sc.typ_l_complex, (2.5, -2.6)::sm_sc.typ_l_complex]], 2))
...
(
  sfunc = sm_sc.fv_arr_cpx_add_ex,
  stype = sm_sc.__typ_arr_cpx_ex,
  initcond = '("{{""(0.0, 0.0)""}}",0)',
  finalfunc = sm_sc.fv_mx_avg_final_cpx,
  parallel = safe
);
...
-- -----------------------------------------------------------------------------------
-- create or replace function sm_sc.fv_array_concat_1d
drop function if exists sm_sc.fv_array_concat_1d(anyarray, anyarray);
create function sm_sc.fv_array_concat_1d
(
  i_left        anyarray,
  l_right       anyarray
)
...
(
  sfunc = sm_sc.fv_array_concat_1d,
  stype = anyarray,
  initcond = '{}',
  parallel = safe
);
