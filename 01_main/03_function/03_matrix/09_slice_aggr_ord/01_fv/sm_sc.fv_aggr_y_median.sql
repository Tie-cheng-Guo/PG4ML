...
create or replace function sm_sc.fv_aggr_y_median
(
  i_array          anyarray
)
returns anyarray
...
begin
  ...
  then
    ...
    (
      select 
        array[array_agg(sm_sc.fv_aggr_slice_median(i_array[ : ][col_a_x : col_a_x]) order by col_a_x)]
