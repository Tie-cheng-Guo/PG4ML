...
create or replace function sm_sc.fv_aggr_y_is_exists_null
(
  i_array          anyarray
)
returns boolean[]
...
begin
  ...
  then
    ...
    (
      select 
        array[array_agg(sm_sc.fv_aggr_slice_is_exists_null(i_array[ : ][col_a_x : col_a_x]) order by col_a_x)]
...
cost 100;
-- -- set search_path to sm_sc;
-- select sm_sc.fv_aggr_y_is_exists_null
--   (
