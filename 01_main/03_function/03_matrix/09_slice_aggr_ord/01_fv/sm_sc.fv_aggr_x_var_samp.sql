...
create or replace function sm_sc.fv_aggr_x_var_samp
(
  i_array          anyarray
)
returns anyarray
...
begin
  ...
  then
    ...
    (
      select 
        array_agg(array[sm_sc.fv_aggr_slice_var_samp(i_array[col_a_y : col_a_y][ : ])] order by col_a_y)
...
cost 100;
-- -- set search_path to sm_sc;
-- select sm_sc.fv_aggr_x_var_samp
--   (
