...
-- drop function if exists sm_sc.fv_apad(anyarray, int, anyarray);
...
begin
  ...
  then 
    ...
    then
      return i_array[array_length(i_array, 1) - i_len + 1 : ];
    else
      return    -- -- --
        (
          select 
  ...
  then 
    ...
    elsif i_len <= array_length(i_array, 1)
    then
      return i_array[array_length(i_array, 2) - i_len + 1 : ][ : ];
    else
      return    -- -- --
        (
          select 
...
begin
  ...
  return sm_sc.fv_apad(i_array, i_len, v_fill_values);
