...
-- ---------------------------
-- drop function if exists sm_sc.fv_eye_arr(variadic sm_sc.__typ_array_ex[]);
create or replace function sm_sc.fv_eye_arr
(
...
begin
  ...
  then
    ...
    cte_shape as
    (
      select col_cur, array_length(i_arrays[col_cur].m_array, 1) as len_y, array_length(i_arrays[col_cur].m_array, 2) as len_x from generate_subscripts(i_arrays, 1) tb(col_cur)
    ),
    -- 每一层方阵大小
    cte_cur_squar as
    (
      select col_cur, sum(least(len_y, len_x)) over(order by col_cur) as cur_squar from cte_shape
    ),
    -- 每一层方阵右下角作为下一层起点
    cte_cur_start_pos as
    (
      select col_cur, (lag(cur_squar :: int, 1, 0) over(order by col_cur)) + 1 as cur_start_pos from cte_cur_squar
    ),
    -- 本层右下角 yx 坐标
    cte_cur_end_pos as
    (
    ...
    -- 所有层坐标的最大值即返回结果矩阵的形状
    select max(cur_end_pos_y), max(cur_end_pos_x) 
    ...
    loop
      ...
      v_cur_pos = v_cur_pos + least(array_length(v_cur_arr.m_array, 1), array_length(v_cur_arr.m_array, 1));
    end loop;
...
parallel safe
  cost 100;
-- select sm_sc.fv_eye_arr(0.5 :: float, array[array[1, 1], array[1, 1]]::float[], array[array[2, 2, 2, 2], array[2, 2, 2, 2], array[2, 2, 2, 2], array[2, 2, 2, 2]]::float[], array[array[3, 3, 3], array[3, 3, 3], array[3, 3, 3]]::float[])
-- select sm_sc.fv_eye_arr(0.8, array[array[1.2, 2.4], array[11.2, 12.4]], array[array[1.3, 2.6, 1.4, 2.8], array[11.3, 12.6, 11.4, 12.8]], array[array[1.1, 2.2], array[11.1, 12.2], array[21.1, 22.2]])
-- ---------------------------
-- drop function if exists sm_sc.fv_eye_arr(variadic sm_sc.__typ_array_ex[]);
create or replace function sm_sc.fv_eye_arr
(
...
-- declare -- here
begin
  return sm_sc.fv_eye_arr(0.0 :: float, variadic i_arrays);
end
$$
  language plpgsql stable
parallel safe
  cost 100;
-- select sm_sc.fv_eye_arr(array[array[1, 1], array[1, 1]]::float[], array[array[2, 2, 2, 2], array[2, 2, 2, 2], array[2, 2, 2, 2], array[2, 2, 2, 2]]::float[], array[array[3, 3, 3], array[3, 3, 3], array[3, 3, 3]]::float[])
-- select sm_sc.fv_eye_arr(array[array[1.2, 2.4], array[11.2, 12.4]], array[array[1.3, 2.6, 1.4, 2.8], array[11.3, 12.6, 11.4, 12.8]], array[array[1.1, 2.2], array[11.1, 12.2], array[21.1, 22.2]])