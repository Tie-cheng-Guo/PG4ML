...
begin
  ...
  -- 审计填充数值矩阵的宽度
  elsif v_fill_values is not null and array_ndims(v_fill_values) = 2 and array_length(v_fill_values, 2) not in (1, array_length(i_array, 2))
  then
  ...
  then
    raise exception 'the slide window with i_period: % and i_window_len: % is not intact at the tail of i_array len: %', i_period, i_window_len, array_length(i_array, 1);
  end if;
...
-- select sm_sc.fv_sample_y
--   (
--     array[array[1, 2], array[3, 4], array[5, 6], array[7, 8], array[9.7, 10], array[11, 12]]
--     , 3
--     , 3
