...
-- drop function if exists sm_sc.fv_opr_turn_back(anyarray);
create or replace function sm_sc.fv_opr_turn_back
(
  i_right     anyarray
)
returns anyarray
...
begin
  ...
  then
    ...
    (
      select
        array_agg(array_x_new order by a_cur_y desc)
      from 
      (
        select 
          a_cur_y,
