...
begin
  ...
  then
    ...
    loop 
      ...
      loop
        i_array[v_y_cur][v_x_cur] := sm_sc.fv_d_gelu(i_array[v_y_cur][v_x_cur]);
        v_x_cur := v_x_cur + 1;
      end loop;
      v_y_cur := v_y_cur + 1;
    end loop;
  ...
  then
    while v_y_cur <= array_length(i_array, 1)
    loop
      i_array[v_y_cur] := sm_sc.fv_d_gelu(i_array[v_y_cur]);
      v_y_cur := v_y_cur + 1;
    end loop;
    return i_array;
...
cost 100;
-- select sm_sc.fv_d_activate_gelu(array[array[1.0 :: float, -2.0], array[3.0, 4.0]])
