    ...
    select 
      ...
      case 
        when sm_sc.tb_fore.node_fn_type = 'agg_concat_y' 
          then sm_sc.fa_array_concat(sm_sc.tb_back.pick_y_idx order by sm_sc.tb_path.path_ord_no) filter(where sm_sc.tb_fore.node_fn_type = 'agg_concat_y')
        when sm_sc.tb_fore.node_fn_type = 'agg_concat_x' 
          then sm_sc.fa_array_concat(distinct sm_sc.tb_back.pick_y_idx) filter(where sm_sc.tb_fore.node_fn_type = 'agg_concat_x')
        -- -- -- 几个输出矩阵高宽规格改变的特殊 fn, 规约按照第一目作为输入数据集高度规格
        -- -- when sm_sc.tb_fore.node_fn_type in ('conv_2d', 'prod_mx')
        else sm_sc.fa_array_concat(case when sm_sc.tb_path.path_ord_no = 1 then sm_sc.tb_back.pick_y_idx end) filter(where sm_sc.tb_fore.node_fn_type not in ('agg_concat_y', 'agg_concat_x'))
      end as pick_y_idx,
      ...
      case
        when sm_sc.tb_fore.node_fn_type in ('agg_sum', 'agg_avg', 'add', 'sub')
          then array[array[max(sm_sc.tb_fore.node_o_len[1]) filter(where sm_sc.tb_fore.node_fn_type in ('agg_sum', 'agg_avg', 'add', 'sub')), max(sm_sc.tb_fore.node_o_len[2]) filter(where sm_sc.tb_fore.node_fn_type in ('agg_sum', 'agg_avg', 'add', 'sub'))]]
        when sm_sc.tb_fore.node_fn_type in ('agg_concat_x', 'agg_concat_y')
          then array_agg(sm_sc.tb_back.node_o_len order by sm_sc.tb_path.path_ord_no) filter(where sm_sc.tb_fore.node_fn_type in ('agg_concat_x', 'agg_concat_y'))
        when sm_sc.tb_fore.node_fn_type = 'slice_x'   -- 暂不支持多区间切片
          then array[array[max(sm_sc.tb_back.node_o_len[1]) filter(where sm_sc.tb_fore.node_fn_type = 'slice_x'), max(sm_sc.tb_back.node_o_len[2]) filter(where sm_sc.tb_fore.node_fn_type = 'slice_x')]]
        when sm_sc.tb_fore.node_fn_type = 'slice_y'   -- 暂不支持多区间切片
          then array[array[max(sm_sc.tb_back.node_o_len[1]) filter(where sm_sc.tb_fore.node_fn_type = 'slice_y'), max(sm_sc.tb_back.node_o_len[2]) filter(where sm_sc.tb_fore.node_fn_type = 'slice_y')]]
