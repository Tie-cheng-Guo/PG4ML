-- 本函数以标量 scalar为入参/出参，数组版本参考：sm_sc.fv_lambda_arr_delta
-- 
        ...
        when 1 
          then -- 优先使用 i_depdt_var 减小算法复杂度 -- -- -1.0 :: float/ (power(log(i_co_value, i_indepdt_var), 2) * i_indepdt_var * ln(i_co_value))
            case
              ...
              else -power(log(i_indepdt_var, i_co_value), 2) / nullif(i_indepdt_var * ln(i_co_value), 0.0 :: float)
            end
        when 2 
