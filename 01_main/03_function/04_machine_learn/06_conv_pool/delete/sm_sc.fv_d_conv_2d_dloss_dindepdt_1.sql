...
-- --   elsif array_length(i_dloss_dy, 1) <> (v_y - v_window_len_y) / i_stride[1] + 1
-- --     or array_length(i_dloss_dy, 2) <> (v_x - v_window_len_x) / i_stride[2] + 1
-- --   then
...
-- --                 sum
-- --                 (
-- --                   coalesce((|~~| i_window)[v_cur_y - ((a_cur_y_dloss_dy - 1) * i_stride[1])][v_cur_x - ((a_cur_x_dloss_dy - 1) * i_stride[2])], 0.0 :: float) 
-- --                     * i_dloss_dy[a_cur_y_dloss_dy][a_cur_x_dloss_dy]     --   dloss_dx 当个元素对应的 dloss_dy 元素      
-- --                     * v_windows_cnt_reciproca                    
-- --                 ) 
-- --               from 
-- --                 -- 以 y 方向为例推导
-- --                 -- 对于 i_array 的第 v_cur_y 个元素，寻找其在 i_dloss_dy 对应的  a_cur_y_dloss_dy 
...
-- --                 generate_series(greatest(ceil((v_cur_y :: decimal - v_window_len_y) / i_stride[1]) + 1, 1), least((v_cur_y - 1) / i_stride[1] + 1, v_y), 1) tb_a_cur_y(a_cur_y_dloss_dy),  
-- --                 generate_series(greatest(ceil((v_cur_x :: decimal - v_window_len_x) / i_stride[2]) + 1, 1), least((v_cur_x - 1) / i_stride[2] + 1, v_x), 1) tb_a_cur_x(a_cur_x_dloss_dy)
-- --             )
-- --             , 0.0
-- --           )
-- --           ;
