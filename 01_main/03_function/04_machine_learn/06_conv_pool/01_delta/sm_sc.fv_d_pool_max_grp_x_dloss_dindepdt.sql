...
(
  i_x_grp_x                float[]                                     ,  -- i_x_grp_x_grp_x[n:n][:] 是 第 n 个 2d 降维为 1d 样本
  ...
  i_y_1d_grp               float[]    default  null                    ,  -- 即已求出的算子结果矩阵
...
begin
  -- 审计二维长度
  if array_ndims(i_x_grp_x) <> 2
  then 
    return null; raise notice 'no method for such length!  Ndim: %; len_1: %; len_2: %;', array_ndims(i_x_grp_x), array_length(i_x_grp_x, 1), array_length(i_x_grp_x, 2);
  elsif array_length(i_x_grp_x, 2) % i_1d_2_2d_cnt_per_grp <> 0
  then 
    ...
    (
      ...
      from generate_series(1, array_length(i_x_grp_x, 1)) tb_a_cur_y(a_cur_y)
    );
  end if;
end
$$
