...
-- drop function sm_sc.fv_mx_max_abs_evd_value(float[][]);
...
declare 
  v_loop_cnt_01             int                   :=   0;
  v_yx_len                  int                   :=   array_length(i_matrix, 1) ;                 -- 方阵维数
								    
  v_cur_array_x             float[]       :=   array_fill(2.0 :: float, array[v_yx_len]); 
...
begin
  ...
  if v_yx_len = array_length(i_matrix, 2)
    -- and /*待处理欠秩、除数为零*/
  then
    ...
    loop 
      ...
      -- v_cur_eigenvalue = (select max(v_cur_array_x[cur]) + v_loop_fasten_factor from generate_series(1, v_yx_len) cur);  -- --
      v_cur_eigenvalue = (select v_cur_array_x[cur] from generate_series(1, v_yx_len) cur order by abs(v_cur_array_x[cur]) limit 1);
      ...
      v_cur_array_x = sm_sc.fv_mx_ele_2d_2_1d(|^~| (i_matrix |**| (|^~| (v_cur_array_x /` v_cur_eigenvalue))));
      -- v_cur_array_x = sm_sc.fv_mx_ele_2d_2_1d(|^~| ((i_matrix -` sm_sc.fv_eye_unit(v_yx_len, v_loop_fasten_factor)) |**| (|^~| (v_cur_array_x /` v_cur_eigenvalue))));  -- --
    end loop;
