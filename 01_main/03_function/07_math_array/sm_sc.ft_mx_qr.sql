...
-- drop function sm_sc.ft_mx_qr(float[][]);
...
declare 
  ...
  v_mx_q                  float[][]    := sm_sc.fv_eye_unit(v_yx_len);  -- q矩阵
...
begin
  ...
  if v_yx_len = array_length(i_matrix, 2)
    -- -- -- and /*待处理欠秩判断？？*/
  then
    while v_loop_cnt_01 < v_yx_len - 1
    loop
      -- 准备本次迭代入参子矩阵
      ...
      -- 自然基底向量
      v_array_e = array_fill(0.0 :: float, array[v_yx_len + 1 - v_loop_cnt_01]);
      v_array_e[1] = 1.0;
      ...
      -- 欧米伽矩阵
      v_mx_w = sm_sc.fv_eye_unit(v_yx_len + 1 - v_loop_cnt_01) -` (2.0 *` ((|^~| array[v_array_w]) |**| array[v_array_w]));

      -- 欧米伽矩阵的左上扩展
      v_mx_w_prepend = sm_sc.fv_area_replace(sm_sc.fv_eye_unit(v_yx_len), array[v_loop_cnt_01, v_loop_cnt_01], v_mx_w);
