...
(
  ...
  -- i_x                                float[][]                       ,      -- [v_len_x1_y1][v_len_x2_w1]                                                         -- 乘法左矩阵，其中自行填写 w0 = 1.0 规约为该入参首列
  -- i_y                                float[][]                       ,      -- [v_len_x1_y1][v_len_w2_y2]                                                         -- 乘法结果
...
$$
declare -- here
-- -- 数学原理：已知矩阵 v_i_x, v_i_y, 且 v_i_x |**| v_ret_w == (v_i_y), 求 v_ret_w
  ...
  v_len_x2_w1                 int               := array_length(v_i_x, 2);
...
raise notice 'step 1 begin: v_y_rand_samp: %', v_y_rand_samp ~=` 4;

    -- 小批量随机梯度下降
    -- -- 小批量取样，降低复杂度和计算量
    select 
      array_agg(array(select unnest(v_i_x[a_y_no : a_y_no][:]))),
...
raise notice 'step 1 end: v_y_rand_samp: %', v_y_rand_samp ~=` 4;
