-- 任务跟踪
select * from sm_sc.tb_classify_task where work_no = -000000019
...
on tb_a_path.fore_node_no = tb_a_node.node_no
  and tb_a_node.work_no = -000000019
where tb_a_node.work_no = -000000019
...
-- 权重参数总量
select 
  sum(cardinality(sm_sc.__fv_get_kv(p_node_depdt)))
from sm_sc.tb_nn_node
where work_no = -000000019
