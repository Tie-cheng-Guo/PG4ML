...
一、准备预训练模型
  1. 导入预训练模型，有如下两种方式任选其一: 
    ...
    b. 第二种方式: 从推理模型生成预训练模型(推理模型的来源，参看 \plpgsql_pg4ml\model\000000016_mnist_fft_bcb_absqrt\02_import_infer_model\readme\): 
      call 
      ...
      -- 准备训练集: 
      delete from sm_sc.tb_nn_train_input_buff where work_no = -000000016; commit;
      copy               
      ...
      from program       'gzip -d < /var/lib/pgsql/model/000000016_mnist_fft_bcb_absqrt/01_import_pre_train_model/model_back_up/sm_sc.tb_nn_train_input_buff_000000016.gz' 
      delimiter          '|' 
      encoding           'UTF8' 
      csv
      header
...
二、验证预训练模型方法参考: 
  plpgsql_pg4ml_dev\model\000000016_mnist_fft_bcb_absqrt\02_import_infer_model\readme\for_test_infer\readme_for_import_test_infer.txt
  第一章 -> 第 2 节